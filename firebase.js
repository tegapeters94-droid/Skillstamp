...
// Track lastSeen on every page load for active user analytics
(function() {
  var _origEnterApp = window.enterApp;
  window.enterApp = function() {
    if (typeof _origEnterApp === 'function') _origEnterApp.apply(this, arguments);
    if (window.ME && window.ME.uid) {
      var now = Date.now();
      window.ME.lastSeen = now;
      window.ME.lastActive = now;
      // Fire-and-forget update to Firestore
      try {
        fbSet('users', window.ME.uid, Object.assign({}, window.ME, { lastSeen: now, lastActive: now })).catch(function(){});
      } catch(e) {}
    }
  };
})();
