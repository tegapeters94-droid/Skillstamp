...
var D = {
  ...
  font: 'font-family:Syne,sans-serif',
};
...
function fHero() {
  ...
  var ring = '<svg width="56" height="56" viewBox="0 0 56 56" style="flex-shrink:0;">'
    ...
    + '<text x="28" y="33" text-anchor="middle" fill="'+color+'" font-family="Syne,sans-serif" font-weight="800" font-size="11">'+pct+'%</text>'
    + '</svg>';
...
function fFAB(role) {
  ...
  var onclick = isClient ? "openPostGig()" : "showPage(\'\1\')";
...
function cHero() {
  ...
  var stats = [
    {val:'$'+balance.toLocaleString(), label:'Balance',    sub: pending ? '$'+pending.toLocaleString()+' escrow' : 'Available', onclick:"showPage(\'\1\')"},
    {val:activeCount,                   label:'Active Gigs', sub:'In progress',                                                    onclick:"showPage(\'\1\')"},
    {val:totalApplicants,               label:'Applicants',  sub:'Awaiting review',                                                onclick:"showPage(\'\1\')"},
  ];
...
function cFeaturedTalent() {
  ...
  talent.forEach(function(u) {
    ...
    html += btnPrimary('💼 Hire', "openHireMe('"+u.uid+"')");
...
// -- CLIENT: Proposals Monitor --
function cProposalsMonitor() {
  var cached = window._ssClientProposals || [];
  // Refresh async on every render
  fbGetAll('proposals').then(function(all) {
    window._ssClientProposals = all.filter(function(p){ return p.fromUid === ME.uid; });
    // Re-render only if still on home
    var pg = document.getElementById('page-home');
    if (pg && pg.classList.contains('active') && window._ssClientProposals.length !== cached.length) renderRoleHome();
  }).catch(function(){});

  var h = '<div style="margin-bottom:16px;">';
  h += sectionHeader('📬 Proposals Sent', 'View all', "showPage('gigs');switchGigTab('proposals')");

  if (!cached.length) {
    h += '<div style="background:var(--s);border:1px solid var(--br);' + D.r16 + ';padding:20px;text-align:center;">';
    h += '<div style="font-size:28px;margin-bottom:8px;">📭</div>';
    h += '<div style="' + D.font + ';font-size:12px;color:var(--td);">No proposals sent yet</div>';
    h += '<div style="font-size:11px;color:var(--td);margin-top:4px;">Browse talent and send a direct proposal.</div>';
    h += '<div style="margin-top:12px;">' + btnPrimary('Browse Talent', "showPage('talent')", '🔍') + '</div>';
    h += '</div></div>';
    return h;
  }

  var statusCfg = {
    pending:  { label:'Pending',  c:'#e8c547', bg:'rgba(232,197,71,.1)',  icon:'⏳' },
    accepted: { label:'Accepted', c:'#4ade80', bg:'rgba(74,222,128,.1)', icon:'✅' },
    rejected: { label:'Declined', c:'#ef4444', bg:'rgba(239,68,68,.08)', icon:'✕'  },
    contract: { label:'Contract', c:'#60a5fa', bg:'rgba(96,165,250,.1)', icon:'📋' }
  };
  var sorted = cached.slice().sort(function(a,b){ return (b.ts||0)-(a.ts||0); });
  h += '<div style="background:var(--s);border:1px solid var(--br);' + D.r16 + ';overflow:hidden;' + D.shadow + ';">';
  sorted.slice(0,4).forEach(function(p, i) {
    var fl = getUser(p.toUid) || { name:'Freelancer', gradient:'#888' };
    var cfg = statusCfg[p.status] || statusCfg.pending;
    var sep = i < Math.min(sorted.length,4)-1 ? 'border-bottom:1px solid var(--br);' : '';
    h += '<div style="display:flex;align-items:center;gap:10px;padding:12px 14px;' + sep + '">';
    h += mkAvatar(fl, 36, '50%');
    h += '<div style="flex:1;min-width:0;">';
    h += '<div style="' + D.font + ';font-weight:700;font-size:12px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">' + p.title + '</div>';
    h += '<div style="font-size:10px;color:var(--td);">To: ' + fl.name + ' · ' + timeAgo(p.ts) + '</div>';
    h += '</div>';
    h += '<span style="font-size:9px;font-weight:700;color:' + cfg.c + ';background:' + cfg.bg + ';padding:3px 8px;border-radius:20px;white-space:nowrap;flex-shrink:0;">' + cfg.icon + ' ' + cfg.label + '</span>';
    h += '</div>';
  });
  h += '</div>';
  if (sorted.length > 4) {
    h += '<div onclick="showPage(\'gigs\');switchGigTab(\'proposals\')" style="text-align:center;font-size:11px;color:var(--gld);font-weight:700;padding:10px;cursor:pointer;">View all ' + sorted.length + ' \u2192</div>';
  }
  h += '</div>';
  return h;
}

// -- CLIENT: Active Workspaces quick-access --
function cWorkspaceLink() {
  var active = getGigs().filter(function(g){
    return g.posterUid === ME.uid && (g.status === 'hired' || g.status === 'delivered');
  });
  if (!active.length) return '';
  var h = '<div style="margin-bottom:16px;">';
  h += sectionHeader('🗂 Active Workspaces', null, '');
  h += '<div style="background:linear-gradient(135deg,rgba(96,165,250,.08),rgba(96,165,250,.03));border:1px solid rgba(96,165,250,.2);' + D.r16 + ';overflow:hidden;">';
  active.slice(0,3).forEach(function(g, i) {
    var fl = getUser(g.hiredUid) || { name:'Freelancer', gradient:'#888' };
    var statusLabel = g.status === 'delivered' ? 'Delivered' : 'In Progress';
    var statusColor = g.status === 'delivered' ? '#4ade80' : '#60a5fa';
    var sep = i < Math.min(active.length,3)-1 ? 'border-bottom:1px solid rgba(96,165,250,.12);' : '';
    h += '<div onclick="openGigWorkspace(\'' + g.id + '\')" style="display:flex;align-items:center;gap:11px;padding:13px 14px;cursor:pointer;' + sep + 'transition:background .15s;" onmouseover="this.style.background=\'rgba(96,165,250,.07)\'" onmouseout="this.style.background=\'\'">';
    h += mkAvatar(fl, 36, '10px');
    h += '<div style="flex:1;min-width:0;">';
    h += '<div style="' + D.font + ';font-weight:700;font-size:12px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">' + g.title + '</div>';
    h += '<div style="font-size:10px;color:var(--td);">with ' + fl.name + ' · <span style="color:' + statusColor + ';font-weight:600;">' + statusLabel + '</span></div>';
    h += '</div>';
    h += '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--td)" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>';
    h += '</div>';
  });
  if (active.length > 3) {
    h += '<div onclick="showPage(\'\1\')" style="text-align:center;font-size:11px;color:var(--gld);font-weight:700;padding:10px;cursor:pointer;border-top:1px solid rgba(96,165,250,.12);">+' + (active.length-3) + ' more →</div>';
  }
  h += '</div></div>';
  return h;
}
