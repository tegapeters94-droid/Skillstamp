...
window.openPostGig = function() {
  ...
  _GW = {
    step: 1,
    data: { title:'', category:'', description:'', skills:[], screeningQ:'', scope:'medium', duration:'1-3 months', budget:'', escrowAgreed:false }
  };
...
function _gwNext() {
  if (!_GW) return;
  var err = _gwValidate();
  if (err) { toast(err, 'bad'); _gwShake(); return; }
  _gwSave();
...
function _gwSave() {
  ...
  if (s === 1) {
    ...
    var c = document.getElementById('gw-cat');
    if (c) {
      d.category = c.value;
      if (!d.category && c.options && c.options.length > 0) {
        d.category = c.options[c.selectedIndex] ? c.options[c.selectedIndex].value : '';
      }
    }
  }
  if (s === 2) {
...
function _gwValidate() {
  ...
  var d = _GW.data, s = _GW.step;
  if (s === 1) {
    if (!d.title || d.title.length < 10) return 'Please enter a title (min 10 characters).';
