...
window.adminEditUser=function(uid){
  ...
  m.innerHTML='<div class="modal-overlay" onclick="closeModal()">'
    ...
    +'<div style="font-family:Syne,sans-serif;font-weight:800;font-size:15px;margin-bottom:14px;">✏ Edit: '+u.name+'</div>'
...
function buildAdminHTML(){
  ...
  function statCard(label,val,col,sub){
    return '<div style="background:var(--s);border:1px solid var(--br);border-radius:var(--r);padding:12px;text-align:center;">'
      +'<div style="font-family:Syne,sans-serif;font-size:22px;font-weight:800;color:'+(col||'var(--fg)')+';">'+val+'</div>'
  ...
  function sHead(icon,label){
    return '<div style="font-family:Syne,sans-serif;font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--td);margin:16px 0 8px;">'+icon+' '+label+'</div>';
  }
  ...
  var h='<div style="display:flex;align-items:center;gap:10px;margin-bottom:18px;">'
    ...
    +'<div><div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;color:var(--acc);">SkillStamp Admin</div>'
    +'<div style="font-size:10px;color:var(--td);">Signed in as '+ME.name+' · Admin Portal</div></div>'
    +'<button onclick="adminExportCSV()" style="margin-left:auto;background:var(--s);border:1px solid var(--br);border-radius:6px;padding:7px 12px;font-size:11px;font-family:Syne,sans-serif;font-weight:700;color:var(--fg);cursor:pointer;">⬇ Export CSV</button>'
    +'</div>';
  ...
  tabs.forEach(function(t){
    ...
    h+='<button id="admt-'+t+'" onclick="adminTab(\''+t+'\')" style="padding:7px 13px;font-size:11px;font-family:Syne,sans-serif;font-weight:700;border-radius:6px;border:1px solid var(--br);background:var(--s);color:var(--td);cursor:pointer;transition:all .15s;">'+tabIcons[t]+' '+tabLabels[t]+badge+'</button>';
  });
  h+='</div>';
  h+='<div id="adm-panel">';
  ...
  function buildUserCards(list){
    ...
    list.forEach(function(u){
      ...
      out+='<div id="ucard-'+u.uid+'" style="background:var(--s);border:1px solid var(--br);border-radius:var(--r);padding:13px;margin-bottom:8px;">'
        ...
        +'<div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,'+u.gradient+','+u.gradient+'88);display:flex;align-items:center;justify-content:center;font-family:Syne,sans-serif;font-weight:800;font-size:13px;color:#fff;flex-shrink:0;">'
  ...
  sortedPosts.slice(0,30).forEach(function(p){
    ...
    h+='<div id="postcrd-'+p.id+'" style="background:var(--s);border:1px solid '+(flagged?'var(--acc)':'var(--br)')+';border-radius:var(--r);padding:11px;margin-bottom:7px;">'
      ...
      +'<div style="font-family:Syne,sans-serif;font-weight:700;font-size:12px;">'+p.userName+'</div>'
  ...
  [...gigs].sort(function(a,b){return (b.created||0)-(a.created||0);}).slice(0,20).forEach(function(g){
    h+='<div id="gigcrd-'+g.id+'" style="background:var(--s);border:1px solid var(--br);border-radius:var(--r);padding:11px;margin-bottom:7px;">'
      ...
      +'<div style="font-family:Syne,sans-serif;font-weight:700;font-size:12px;flex:1;">'+g.title+'</div>'
  ...
  pendingList.forEach(function(p){
    ...
    h+='<div style="background:var(--s);border:1px solid var(--br);border-radius:var(--r);padding:14px;margin-bottom:9px;">'
      ...
      +'<div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:12px;">'+submitter.name+'</div>'
      ...
      +'<button onclick="adminApprove(\''+p.id+'\')" style="flex:1;padding:8px;font-size:11px;font-family:Syne,sans-serif;font-weight:700;background:var(--grn);color:#fff;border:none;border-radius:6px;cursor:pointer;">✓ Approve</button>'
      +'<button onclick="adminReject(\''+p.id+'\')" style="flex:1;padding:8px;font-size:11px;font-family:Syne,sans-serif;font-weight:700;background:rgba(255,107,53,.15);color:var(--acc);border:1px solid var(--acc);border-radius:6px;cursor:pointer;">✕ Reject</button>'
  ...
  byLogin.slice(0,12).forEach(function(u,i){
    ...
    h+='<div style="flex:1;min-width:0;"><div style="font-family:Syne,sans-serif;font-weight:600;font-size:11px;">'+u.name+'</div><div style="font-size:9px;color:var(--td);">'+u.role+' · '+u.country+'</div></div>';
  ...
  h+='<div style="background:var(--s);border:1px solid var(--br);border-radius:var(--r);padding:14px;margin-bottom:14px;">'
    ...
    +'<button onclick="adminExportCSV()" style="padding:10px;font-size:12px;font-family:Syne,sans-serif;font-weight:700;border:1px solid var(--br);border-radius:6px;background:var(--s);color:var(--fg);cursor:pointer;text-align:left;">⬇ Users CSV — '+users.length+' users</button>'
    +'<button onclick="adminExportPostsCSV()" style="padding:10px;font-size:12px;font-family:Syne,sans-serif;font-weight:700;border:1px solid var(--br);border-radius:6px;background:var(--s);color:var(--fg);cursor:pointer;text-align:left;">⬇ Posts CSV — '+posts.length+' posts</button>'
    +'<button onclick="adminExportGigsCSV()" style="padding:10px;font-size:12px;font-family:Syne,sans-serif;font-weight:700;border:1px solid var(--br);border-radius:6px;background:var(--s);color:var(--fg);cursor:pointer;text-align:left;">⬇ Gigs CSV — '+gigs.length+' gigs</button>'
    +'</div></div>';
...
function adminStat(label,val,col){
  return '<div style="background:var(--s);border:1px solid var(--br);border-radius:var(--r);padding:12px;text-align:center;">'
    +'<div style="font-family:Syne,sans-serif;font-weight:800;font-size:20px;color:'+(col||'var(--gld)')+';">'+val+'</div>'
...
function buildUsersList(users){
  ...
  for(var i=0;i<users.length;i++){
    ...
    h+='<div style="background:var(--s);border:1px solid '+(isBanned?'rgba(255,107,53,.4)':'var(--br)')+';border-radius:var(--r);padding:12px;margin-bottom:8px;">'
      ...
      +'<div style="width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,'+u.gradient+','+u.gradient+'88);display:flex;align-items:center;justify-content:center;font-family:Syne,sans-serif;font-weight:700;font-size:12px;flex-shrink:0;">'+initials(u.name)+'</div>'
      +'<div style="flex:1;min-width:0;">'
      +'<div style="font-weight:700;font-size:12px;font-family:Syne,sans-serif;">'+u.name+(u.isAdmin?' <span style="font-size:9px;background:rgba(255,107,53,.15);border:1px solid rgba(255,107,53,.3);color:var(--acc);padding:1px 5px;border-radius:6px;">ADMIN</span>':'')+'</div>'
...
function adminSearchV6(q){
  ...
  for(var i=0;i<Math.min(fil.length,50);i++){
    ...
    h+='<div style="display:flex;align-items:center;gap:8px;padding:9px;background:var(--s);border:1px solid '+(isBanned?'rgba(255,107,53,.3)':'var(--br)')+';border-radius:var(--r);margin-bottom:5px;">'
      +avHTML(u,30,'50%')
      +'<div style="flex:1;min-width:0;"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+u.name+'</div>'
...
window.adminViewDetail = function(uid) {
  ...
  var avEl = u.avatar
    ...
    : '<div style="width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,' + u.gradient + ',' + u.gradient + '88);display:flex;align-items:center;justify-content:center;font-family:Syne,sans-serif;font-weight:800;font-size:18px;color:#000;">' + initials(u.name) + '</div>';
  ...
  h += '<div><div style="font-family:Syne,sans-serif;font-weight:800;font-size:15px;">' + u.name + '</div>';
  ...
  h += '<div style="background:var(--s2);border-radius:8px;padding:10px;text-align:center;"><div style="font-family:Syne,sans-serif;font-weight:800;font-size:14px;color:var(--grn);">$' + Math.round(w.balance||0).toLocaleString() + '</div><div style="font-size:9px;color:var(--td);">Balance</div></div>';
  h += '<div style="background:var(--s2);border-radius:8px;padding:10px;text-align:center;"><div style="font-family:Syne,sans-serif;font-weight:800;font-size:14px;">$' + Math.round(w.earned||0).toLocaleString() + '</div><div style="font-size:9px;color:var(--td);">Earned</div></div>';
  h += '<div style="background:var(--s2);border-radius:8px;padding:10px;text-align:center;"><div style="font-family:Syne,sans-serif;font-weight:800;font-size:14px;">' + (u.gigsCount||0) + '</div><div style="font-size:9px;color:var(--td);">Gigs</div></div>';
  h += '</div>';
  ...
  if (txns.length) {
    ...
    txns.forEach(function(t,i) {
      ...
      h += '<div style="flex:1;min-width:0;"><div style="font-family:Syne,sans-serif;font-size:11px;font-weight:600;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">' + (t.desc||'Transaction') + '</div>';
      h += '<div style="font-size:9px;color:var(--td);">' + timeAgo(t.ts) + '</div></div>';
      h += '<div style="font-family:Syne,sans-serif;font-weight:700;font-size:12px;color:' + (isIn?'var(--grn)':'var(--acc)') + ';flex-shrink:0;">' + (isIn?'+':'-') + '$' + Math.round(t.amount||0).toLocaleString() + '</div>';
      h += '</div>';
    });
    h += '</div>';
  }
  ...
  h += '<button onclick="adminDeleteUserPermanently(\'' + uid + '\',\'' + encodeURIComponent(u.name) + '\')" style="width:100%;padding:10px;background:rgba(239,68,68,.1);border:1.5px solid rgba(239,68,68,.3);color:#ef4444;font-family:Syne,sans-serif;font-weight:700;font-size:12px;border-radius:8px;cursor:pointer;">🗑 Delete Account Permanently</button>';
  h += '</div>';
  ...
  try {
    ...
    el.innerHTML = tickets.map(function(t) {
      ...
      return '<div style="background:var(--s);border:1px solid var(--br);border-radius:var(--r);padding:13px;margin-bottom:8px;">'
        ...
        + '<div style="flex:1;"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:12px;color:' + cfg.c + ';">' + cfg.label + '</div>'
  ...
  try {
    ...
    el.innerHTML = reports.map(function(r) {
      ...
      return '<div style="background:var(--s);border:1px solid var(--br);border-radius:var(--r);padding:13px;margin-bottom:8px;">'
        ...
        + '<div style="flex:1;min-width:0;"><div style="font-family:Syne,sans-serif;font-weight:700;font-size:12px;">Reported: ' + reported.name + '</div>'
...
window.adminDeleteUserPermanently = async function(uid, encodedName) {
  ...
  setModal(
    ...
    + '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;color:#ef4444;margin-bottom:6px;">Delete Account</div>'
