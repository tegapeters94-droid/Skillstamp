...
function _renderReceivedProposals(el, proposals) {
  if (!proposals.length) {
    el.innerHTML =
      ...
      + '<div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;margin-bottom:6px;color:var(--tx);">No proposals yet</div>'
  ...
  el.innerHTML = '<div style="padding:16px 12px 4px;">'
    + '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:13px;margin-bottom:14px;color:var(--tx);">📬 Proposals Received</div>'
    + sorted.map(function(p) {
        ...
        return '<div style="background:var(--s);border:1px solid var(--br);border-radius:14px;margin-bottom:12px;overflow:hidden;" id="prop-card-' + p.id + '">'
          ...
          + '<div style="font-family:Syne,sans-serif;font-weight:700;font-size:13px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">' + p.title + '</div>'
          ...
          + '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:14px;color:var(--grn);">$' + (p.budget||0).toLocaleString() + '</div>'
          + '<div style="font-size:9px;color:var(--td);text-transform:uppercase;letter-spacing:.05em;">Budget</div>'
          + '</div>'
          + '<div style="background:var(--s2);border-radius:8px;padding:8px 12px;flex:1;text-align:center;">'
          + '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:14px;">'+p.timeline+'</div>'
          ...
          + (p.status === 'pending'
            ? '<div style="display:flex;gap:8px;">'
              + '<button onclick="acceptProposal(\'' + p.id + '\')" style="flex:1;padding:11px;background:linear-gradient(135deg,#16a34a,#15803d);color:#fff;font-family:Syne,sans-serif;font-weight:700;font-size:12px;border:none;border-radius:10px;cursor:pointer;">✓ Accept Proposal</button>'
              + '<button onclick="rejectProposal(\'' + p.id + '\')" style="padding:11px 16px;background:var(--s2);border:1px solid var(--br);color:var(--td);font-family:Syne,sans-serif;font-weight:600;font-size:12px;border-radius:10px;cursor:pointer;">Decline</button>'
              + '</div>'
            : p.status === 'accepted'
...
function _renderSentProposals(el, proposals) {
  ...
  if (!proposals.length) {
    el.innerHTML =
      '<div style="padding:48px 20px;text-align:center;">'
      + '<div style="font-size:40px;margin-bottom:12px;opacity:.3;">📤</div>'
      + '<div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;margin-bottom:6px;color:var(--tx);">No proposals sent</div>'
  ...
  el.innerHTML = '<div style="padding:16px 12px 4px;">'
    ...
    + '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:13px;color:var(--tx);">📤 Proposals Sent</div>'
...
window.sendDirectProposalToHub = async function(toUid, title, description, budget, timeline, referenceImages) {
  ...
  // Notify freelancer — isolated so failures don't break the proposal save
  try {
    await pushNotif(
      toUid, 'proposal',
      '📋 New Direct Proposal',
  ...
  } catch(notifErr) { console.warn('pushNotif failed (non-critical):', notifErr); }
  try {
    await sendAutoMsg(
      toUid,
      '📋 Direct Proposal from ' + ME.name + '\n\nProject: ' + title + '\nBudget: $' + budget + '\nTimeline: ' + timeline + '\n\nGo to Gigs > Proposals tab to review and accept.'
    );
  } catch(msgErr) { console.warn('sendAutoMsg failed (non-critical):', msgErr); }
