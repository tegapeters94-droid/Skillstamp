...
window.switchGigTab=function(tab){
  _gigTab=tab;
  var browseTab=document.getElementById("gtab-browse");
  if(browseTab) browseTab.classList.toggle("active",tab==="browse");
  ...
  // Show/hide the Open Gigs section (header + cats + list) — only visible on browse tab
  var gigsSection=document.querySelector('#page-gigs .section');
  if(gigsSection) gigsSection.style.display=(tab==="browse")?'':'none';
...
// ── CLIENT GIG TAB SWITCHER ───────────────────────────────────
var _clientGigTab = 'mygigs';
window.switchClientTab = function(tab) {
  _clientGigTab = tab;
  var myGigsBtn = document.getElementById('ctab-mygigs');
  var propsBtn  = document.getElementById('ctab-proposals');
  if (myGigsBtn) myGigsBtn.classList.toggle('active', tab === 'mygigs');
  if (propsBtn)  propsBtn.classList.toggle('active',  tab === 'proposals');

  // Show/hide the gigs section (header + list) — only for mygigs tab
  var gigsSection = document.querySelector('#page-gigs .section');
  if (gigsSection) gigsSection.style.display = tab === 'mygigs' ? '' : 'none';

  // Clear dynamic content area
  var ca = document.getElementById('gig-content-area');
  if (ca) ca.innerHTML = '';

  if (tab === 'mygigs')     renderGigs();
  else if (tab === 'proposals') renderProposalHub();
};
...
window.openGigWorkspace=async function(gid){
  ...
  setTimeout(function(){
    ...
    // Wire message button — single handler, no race conditions
    var _chatUid = isClient ? gig.hiredUid : gig.posterUid;
    var msgBtn = document.createElement('button');
    msgBtn.className = 'btn2';
    msgBtn.id = 'ws-msg';
    msgBtn.style.cssText = 'width:100%;margin-top:8px;font-size:11px;';
    msgBtn.textContent = 'Message ' + (isClient ? freelancer.name : client.name);
    var _msgFired = false;
    function _doOpenChat(e) {
      if (e) { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation(); }
      if (_msgFired) return; // prevent double-fire from touchend+click
      _msgFired = true;
      if (!_chatUid) { toast('Cannot open chat — gig not yet hired.', 'bad'); _msgFired = false; return; }
      window._chatOpening = true;
      // Directly render chat into mcontent without going through modal open/close cycle
      var mc = document.getElementById('mcontent');
      var ov = document.getElementById('moverlay');
      if (mc) mc.style.cssText = 'position:fixed;inset:0;width:100%;height:100%;max-width:100%;max-height:100%;margin:0;padding:0;border-radius:0;border:none;overflow:hidden;z-index:1001;background:var(--s);';
      if (ov) { ov.style.padding = '0'; ov.classList.add('show'); }
      // Show loading state
      if (mc) mc.innerHTML = '<div style="display:flex;flex-direction:column;height:100%;position:absolute;inset:0;">'
        ...
        + '<button onclick="closeModal()" style="background:none;border:none;color:var(--tx);font-size:22px;cursor:pointer;line-height:1;padding:0 8px 0 0;">←</button>'
        + '<div style="font-size:13px;font-weight:700;color:var(--tx);">Opening chat…</div>'
        + '</div>'
        + '<div style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--td);font-size:12px;">Loading messages…</div>'
        + '</div>';
      // Open the real chat — openMsg will take over from here
      setTimeout(function() {
        window._chatOpening = false;
        openMsg(_chatUid);
      }, 50);
    }
    msgBtn.addEventListener('touchstart', function(e) { e.stopPropagation(); }, {passive: false});
    msgBtn.addEventListener('touchend', _doOpenChat, {passive: false});
    msgBtn.addEventListener('click', _doOpenChat);
...
window.showPage = function(name, skipHistory) {
  ...
  if (name === 'gigs') {
    loadGigsToCache().then(function() {
      ...
      // Always restore the gigs section visibility
      var gigsSection = document.querySelector('#page-gigs .section');
      if (gigsSection) gigsSection.style.display = '';

      if (isClient) {
        // Show client tab bar, hide freelancer tab bar
        var clientBar = document.getElementById('client-gig-tabs-bar');
        var freeBar = document.getElementById('gig-tabs-bar');
        if (clientBar) clientBar.style.display = 'flex';
        if (freeBar) freeBar.style.display = 'none';

        // Update client proposals badge
        fbGetAll('proposals').then(function(all) {
          var pending = all.filter(function(p){ return p.toUid===ME.uid && p.status==='pending'; }).length;
          var cpc = document.getElementById('client-proposals-count');
          if (cpc) { cpc.textContent = pending; cpc.style.display = pending ? '' : 'none'; }
        }).catch(function(){});

        // Default to mygigs tab for clients
        switchClientTab('mygigs');

      } else {
        // Show freelancer tab bar, hide client tab bar
        var freeBar2 = document.getElementById('gig-tabs-bar');
        var clientBar2 = document.getElementById('client-gig-tabs-bar');
        if (freeBar2) freeBar2.style.display = 'flex';
        if (clientBar2) clientBar2.style.display = 'none';

        // My Applications tab — freelancers only
        var myappsTab = document.getElementById('gtab-myapps');
        if (myappsTab) myappsTab.style.display = isFreelancer ? '' : 'none';
        ...
        fbGetAll('proposals').then(function(all) {
          var mine = all.filter(function(p){ return p.toUid===ME.uid && p.status==='pending'; });
          var pc = document.getElementById('proposals-count');
          if (pc) { pc.textContent = mine.length; pc.style.display = mine.length ? '' : 'none'; }
        }).catch(function(){});

        if (_gigTab === 'myapps' && isFreelancer) renderMyApplications();
