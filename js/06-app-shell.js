...
function checkProfileComplete(){
  ...
  var total=4, done=total-missing.length, pct=Math.round((done/total)*100);
  var tipMap={
    bio:{icon:'📝',label:'Add a bio',desc:'Let clients know who you are and what you do.'},
    skills:{icon:'⚡',label:'Add your skills',desc:'Get discovered by clients looking for your expertise.'},
    photo:{icon:'📸',label:'Upload a photo',desc:'Profiles with photos earn 4× more trust.'},
    title:{icon:'🏷️',label:'Set your title',desc:'Your professional headline — make it count.'}
  };
  var itemsHTML=missing.slice(0,3).map(function(k){
    var t=tipMap[k];
    return '<div style="display:flex;align-items:center;gap:12px;padding:11px 0;border-bottom:1px solid rgba(255,255,255,.06);">'
      +'<div style="width:34px;height:34px;border-radius:10px;background:rgba(26,107,60,.18);display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;">'+t.icon+'</div>'
      +'<div style="flex:1;">'
      +'<div style="font-family:Plus Jakarta Sans,sans-serif;font-weight:700;font-size:12px;color:var(--tx);">'+t.label+'</div>'
      +'<div style="font-size:10px;color:var(--td);line-height:1.4;margin-top:1px;">'+t.desc+'</div>'
      +'</div>'
      +'<div style="width:20px;height:20px;border-radius:50%;border:1.5px solid rgba(255,255,255,.15);flex-shrink:0;"></div>'
      +'</div>';
  }).join('');

  var arcR=22, arcC=2*Math.PI*arcR, arcDash=(pct/100)*arcC;

  var html=
    '<button class="mclose" id="pc-close" style="z-index:2;">✕</button>'
    +'<div style="position:relative;padding:0 0 6px;">'
    +'<div style="background:linear-gradient(135deg,rgba(26,107,60,.22) 0%,rgba(26,107,60,.06) 100%);border-radius:14px;padding:18px 16px 14px;margin-bottom:16px;border:1px solid rgba(26,107,60,.25);backdrop-filter:blur(8px);">'
    +'<div style="display:flex;align-items:center;gap:14px;">'
    +'<div style="position:relative;width:56px;height:56px;flex-shrink:0;">'
    +'<svg width="56" height="56" viewBox="0 0 56 56" style="transform:rotate(-90deg);">'
    +'<circle cx="28" cy="28" r="'+arcR+'" fill="none" stroke="rgba(255,255,255,.08)" stroke-width="4"/>'
    +'<circle cx="28" cy="28" r="'+arcR+'" fill="none" stroke="#1a6b3c" stroke-width="4" stroke-linecap="round" stroke-dasharray="'+arcDash+' '+arcC+'"/>'
    +'</svg>'
    +'<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:Plus Jakarta Sans,sans-serif;font-weight:800;font-size:13px;color:var(--tx);">'+pct+'%</div>'
    +'</div>'
    +'<div style="flex:1;">'
    +'<div style="font-family:Plus Jakarta Sans,sans-serif;font-weight:800;font-size:15px;color:var(--tx);margin-bottom:3px;">Unlock Your Full Potential</div>'
    +'<div style="font-size:11px;color:var(--td);line-height:1.5;">A complete profile gets <strong style="color:#4ade80;">3× more views</strong> from premium clients.</div>'
    +'</div>'
    +'</div>'
    +'</div>'
    +itemsHTML
    +'<div style="display:flex;gap:8px;margin-top:16px;">'
    +'<button id="pc-go" style="flex:1;padding:13px;background:linear-gradient(135deg,#1a6b3c,#0f4a28);color:#fff;font-family:Plus Jakarta Sans,sans-serif;font-weight:800;font-size:12px;border:none;border-radius:12px;cursor:pointer;letter-spacing:.01em;">Complete Profile →</button>'
    +'<button id="pc-later" style="padding:13px 16px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:var(--td);font-family:Plus Jakarta Sans,sans-serif;font-weight:600;font-size:11px;border-radius:12px;cursor:pointer;">Later</button>'
    +'</div>'
    +'</div>';
...
var _lottieLoaded=false;
function loadLottie(cb){
  if(window.lottie){cb();return;}
  if(_lottieLoaded){var t=setInterval(function(){if(window.lottie){clearInterval(t);cb();}},80);return;}
  _lottieLoaded=true;
...
function _lottieMakeAnim(layers){
  return {v:'5.5.7',fr:60,ip:0,op:180,w:200,h:200,layers:layers};
}
function _lottieGlobe(){
  return _lottieMakeAnim([
    {ty:4,nm:'hub',ks:{o:{a:0,k:100},r:{a:0,k:0},p:{a:0,k:[100,100,0]},a:{a:0,k:[0,0,0]},s:{a:1,k:[{t:0,s:[80,80,100],e:[100,100,100],i:{x:[.4],y:[1]},o:{x:[.6],y:[0]}},{t:90,s:[100,100,100],e:[80,80,100],i:{x:[.4],y:[1]},o:{x:[.6],y:[0]}},{t:180,s:[80,80,100]}]}},
    shapes:[{ty:'el',p:{a:0,k:[0,0]},s:{a:0,k:[28,28]}},{ty:'fl',c:{a:0,k:[0.1,0.42,0.24,1]},o:{a:0,k:100}}],ip:0,op:180,st:0},
    {ty:4,nm:'ring',ks:{o:{a:0,k:60},r:{a:1,k:[{t:0,s:[0],e:[360],i:{x:[.5],y:[1]},o:{x:[.5],y:[0]}},{t:180,s:[360]}]},p:{a:0,k:[100,100,0]},a:{a:0,k:[0,0,0]},s:{a:0,k:[100,100,100]}},
    shapes:[{ty:'el',p:{a:0,k:[0,0]},s:{a:0,k:[110,110]}},{ty:'st',c:{a:0,k:[0.1,0.42,0.24,1]},o:{a:0,k:100},w:{a:0,k:2},lc:1,lj:1,d:[{n:'d',nm:'dash',v:{a:0,k:8}},{n:'g',nm:'gap',v:{a:0,k:6}}]}],ip:0,op:180,st:0},
    {ty:4,nm:'n1',ks:{o:{a:0,k:100},r:{a:0,k:0},p:{a:1,k:[{t:0,s:[145,60,0],e:[155,50,0],i:{x:[.4],y:[1]},o:{x:[.6],y:[0]}},{t:90,s:[155,50,0],e:[145,60,0],i:{x:[.4],y:[1]},o:{x:[.6],y:[0]}},{t:180,s:[145,60,0]}]},a:{a:0,k:[0,0,0]},s:{a:0,k:[100,100,100]}},
    shapes:[{ty:'el',p:{a:0,k:[0,0]},s:{a:0,k:[16,16]}},{ty:'fl',c:{a:0,k:[0.91,0.77,0.28,1]},o:{a:0,k:100}}],ip:0,op:180,st:0},
    {ty:4,nm:'n2',ks:{o:{a:0,k:100},r:{a:0,k:0},p:{a:1,k:[{t:0,s:[60,50,0],e:[50,60,0],i:{x:[.4],y:[1]},o:{x:[.6],y:[0]}},{t:90,s:[50,60,0],e:[60,50,0],i:{x:[.4],y:[1]},o:{x:[.6],y:[0]}},{t:180,s:[60,50,0]}]},a:{a:0,k:[0,0,0]},s:{a:0,k:[100,100,100]}},
    shapes:[{ty:'el',p:{a:0,k:[0,0]},s:{a:0,k:[16,16]}},{ty:'fl',c:{a:0,k:[0.91,0.77,0.28,1]},o:{a:0,k:100}}],ip:0,op:180,st:0}
  ]);
}
function _lottieScan(){
  return _lottieMakeAnim([
    {ty:4,nm:'card',ks:{o:{a:0,k:100},r:{a:0,k:0},p:{a:0,k:[100,110,0]},a:{a:0,k:[0,0,0]},s:{a:0,k:[100,100,100]}},
    shapes:[{ty:'rc',p:{a:0,k:[0,0]},s:{a:0,k:[120,100]},r:{a:0,k:12}},{ty:'fl',c:{a:0,k:[0.08,0.13,0.09,1]},o:{a:0,k:100}},{ty:'st',c:{a:0,k:[0.1,0.42,0.24,1]},o:{a:0,k:100},w:{a:0,k:2},lc:1,lj:1}],ip:0,op:180,st:0},
    {ty:4,nm:'scan',ks:{o:{a:1,k:[{t:0,s:[0],e:[100]},{t:20,s:[100],e:[100]},{t:160,s:[100],e:[0]},{t:180,s:[0]}]},r:{a:0,k:0},p:{a:1,k:[{t:0,s:[100,62,0],e:[100,158,0],i:{x:[.5],y:[1]},o:{x:[.5],y:[0]}},{t:90,s:[100,62,0],e:[100,158,0],i:{x:[.5],y:[1]},o:{x:[.5],y:[0]}},{t:180,s:[100,62,0]}]},a:{a:0,k:[0,0,0]},s:{a:0,k:[100,100,100]}},
    shapes:[{ty:'rc',p:{a:0,k:[0,0]},s:{a:0,k:[116,2]},r:{a:0,k:1}},{ty:'fl',c:{a:0,k:[0.29,0.87,0.5,1]},o:{a:0,k:80}}],ip:0,op:180,st:0}
  ]);
}
function _lottieShield(){
  return _lottieMakeAnim([
    {ty:4,nm:'shield',ks:{o:{a:0,k:100},r:{a:0,k:0},p:{a:0,k:[100,105,0]},a:{a:0,k:[0,0,0]},s:{a:1,k:[{t:0,s:[90,90,100],e:[100,100,100],i:{x:[.34,1,.64,1],y:[1]},o:{x:[.6],y:[0]}},{t:30,s:[100,100,100],e:[96,96,100],i:{x:[.4],y:[1]},o:{x:[.6],y:[0]}},{t:90,s:[96,96,100],e:[100,100,100],i:{x:[.4],y:[1]},o:{x:[.6],y:[0]}},{t:180,s:[100,100,100]}]}},
    shapes:[{ty:'el',p:{a:0,k:[0,0]},s:{a:0,k:[80,80]}},{ty:'fl',c:{a:0,k:[0.06,0.17,0.1,1]},o:{a:0,k:100}},{ty:'st',c:{a:1,k:[{t:0,s:[0.1,0.42,0.24,1],e:[0.29,0.87,0.5,1],i:{x:[.5],y:[1]},o:{x:[.5],y:[0]}},{t:90,s:[0.29,0.87,0.5,1],e:[0.1,0.42,0.24,1],i:{x:[.5],y:[1]},o:{x:[.5],y:[0]}},{t:180,s:[0.1,0.42,0.24,1]}]},o:{a:0,k:100},w:{a:0,k:4},lc:1,lj:1}],ip:0,op:180,st:0},
    {ty:4,nm:'glow',ks:{o:{a:1,k:[{t:0,s:[0],e:[40]},{t:40,s:[40],e:[0]},{t:80,s:[0],e:[40]},{t:120,s:[40],e:[0]},{t:180,s:[0]}]},r:{a:0,k:0},p:{a:0,k:[100,105,0]},a:{a:0,k:[0,0,0]},s:{a:0,k:[100,100,100]}},
    shapes:[{ty:'el',p:{a:0,k:[0,0]},s:{a:0,k:[120,120]}},{ty:'st',c:{a:0,k:[0.29,0.87,0.5,1]},o:{a:0,k:100},w:{a:0,k:2},lc:1,lj:1}],ip:0,op:180,st:0}
  ]);
}
function _lottieBadge(){
  return _lottieMakeAnim([
    {ty:4,nm:'card',ks:{o:{a:0,k:100},r:{a:0,k:0},p:{a:0,k:[100,100,0]},a:{a:0,k:[0,0,0]},s:{a:0,k:[100,100,100]}},
    shapes:[{ty:'rc',p:{a:0,k:[0,0]},s:{a:0,k:[130,80]},r:{a:0,k:14}},{ty:'fl',c:{a:0,k:[0.07,0.11,0.08,1]},o:{a:0,k:100}},{ty:'st',c:{a:0,k:[0.15,0.5,0.28,1]},o:{a:0,k:100},w:{a:0,k:1.5},lc:1,lj:1}],ip:0,op:180,st:0},
    {ty:4,nm:'stamp',ks:{o:{a:1,k:[{t:60,s:[0],e:[100]},{t:90,s:[100]}]},r:{a:1,k:[{t:60,s:[20],e:[0],i:{x:[.34],y:[1]},o:{x:[.6],y:[0]}},{t:90,s:[0]}]},p:{a:0,k:[100,100,0]},a:{a:0,k:[0,0,0]},s:{a:1,k:[{t:60,s:[140,140,100],e:[100,100,100],i:{x:[.34,1,.64,1],y:[1]},o:{x:[.6],y:[0]}},{t:90,s:[100,100,100]}]}},
    shapes:[{ty:'el',p:{a:0,k:[0,0]},s:{a:0,k:[56,56]}},{ty:'fl',c:{a:0,k:[0.1,0.42,0.24,1]},o:{a:0,k:100}}],ip:0,op:180,st:0},
    {ty:4,nm:'glow',ks:{o:{a:1,k:[{t:80,s:[0],e:[60]},{t:110,s:[60],e:[0]},{t:140,s:[0]}]},r:{a:0,k:0},p:{a:0,k:[100,100,0]},a:{a:0,k:[0,0,0]},s:{a:0,k:[100,100,100]}},
    shapes:[{ty:'el',p:{a:0,k:[0,0]},s:{a:0,k:[90,90]}},{ty:'st',c:{a:0,k:[0.91,0.77,0.28,1]},o:{a:0,k:100},w:{a:0,k:3},lc:1,lj:1}],ip:0,op:180,st:0}
  ]);
}
...
function _buildCinematicOnboarding(){
  ...
  var pages=[
    {id:'ob-p0',lottie:_lottieGlobe,tag:'The Platform',headline:'Welcome to the<br>Future of Work.',sub:'SkillStamp connects Africa\'s elite digital talent with global opportunities through a high-trust, verified ecosystem.'},
    {id:'ob-p1',lottie:_lottieScan,tag:'Smart Matching',headline:'Precision<br>Matching.',sub:'No more bidding wars. Our engine pairs your exact skills to the right gigs — instantly and intelligently.'},
    {id:'ob-p2',lottie:_lottieShield,tag:'Security',headline:'Work with<br>Peace of Mind.',sub:'Your payments are protected. Our Communication Guard and Escrow system ensure you get paid for every milestone.'},
    {id:'ob-p3',lottie:_lottieBadge,tag:'Verification',headline:'Stand Out<br>from the Crowd.',sub:'Earn your SkillID badge and unlock 3× more visibility from premium clients worldwide.'},
    {id:'ob-p4',lottie:null,tag:'Get Started',headline:'Ready to Build<br>Your Legacy?',sub:'Join thousands of verified African professionals. Your first gig is waiting.',isCta:true}
  ];

  if(!document.getElementById('ob-style')){
    var st=document.createElement('style');
    st.id='ob-style';
    st.textContent='#ob-cinematic{position:fixed;inset:0;z-index:99990;background:#07100a;overflow:hidden;font-family:"Plus Jakarta Sans",sans-serif;display:flex;flex-direction:column;touch-action:pan-y;}'
      +'.ob-track{display:flex;flex-direction:row;width:500%;height:100%;transition:transform .42s cubic-bezier(.4,0,.2,1);will-change:transform;}'
      +'.ob-page{width:20%;height:100%;flex-shrink:0;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;padding:0 0 40px;position:relative;overflow:hidden;}'
      +'.ob-bg-orb{position:absolute;top:-30%;left:50%;transform:translateX(-50%);width:340px;height:340px;border-radius:50%;background:radial-gradient(circle,rgba(26,107,60,.28) 0%,transparent 70%);pointer-events:none;}'
      +'.ob-lottie-wrap{position:absolute;top:0;left:0;right:0;bottom:45%;display:flex;align-items:center;justify-content:center;pointer-events:none;}'
      +'.ob-lottie-con{width:180px;height:180px;filter:drop-shadow(0 0 32px rgba(26,107,60,.5));}'
      +'.ob-content{position:relative;z-index:2;width:100%;padding:0 28px;text-align:center;}'
      +'.ob-tag{display:inline-block;font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#4ade80;background:rgba(74,222,128,.1);border:1px solid rgba(74,222,128,.2);border-radius:20px;padding:4px 12px;margin-bottom:16px;}'
      +'.ob-headline{font-size:clamp(26px,7vw,32px);font-weight:800;line-height:1.18;color:#f0ede6;letter-spacing:-.02em;margin-bottom:14px;}'
      +'.ob-sub{font-size:14px;font-weight:400;line-height:1.65;color:rgba(240,237,230,.55);max-width:320px;margin:0 auto 28px;}'
      +'.ob-dots{display:flex;gap:6px;align-items:center;justify-content:center;margin-bottom:20px;}'
      +'.ob-dot{width:6px;height:6px;border-radius:3px;background:rgba(255,255,255,.18);transition:all .3s ease;}'
      +'.ob-dot.active{width:22px;background:#1a6b3c;}'
      +'.ob-nav{display:flex;align-items:center;justify-content:space-between;padding:0 28px;width:100%;box-sizing:border-box;}'
      +'.ob-skip{background:none;border:none;color:rgba(240,237,230,.35);font-family:"Plus Jakarta Sans",sans-serif;font-size:12px;font-weight:500;cursor:pointer;padding:10px 0;}'
      +'.ob-next{background:linear-gradient(135deg,#1a6b3c,#0f4a28);border:none;color:#fff;font-family:"Plus Jakarta Sans",sans-serif;font-weight:800;font-size:13px;padding:13px 28px;border-radius:50px;cursor:pointer;box-shadow:0 4px 24px rgba(26,107,60,.5);letter-spacing:.01em;transition:transform .15s;}'
      +'.ob-next:active{transform:scale(.96);}'
      +'.ob-cta-wrap{width:100%;padding:0 28px;box-sizing:border-box;display:flex;flex-direction:column;gap:10px;margin-bottom:12px;}'
      +'.ob-cta-main{width:100%;padding:15px;background:linear-gradient(135deg,#1a6b3c,#0f4a28);border:none;color:#fff;font-family:"Plus Jakarta Sans",sans-serif;font-weight:800;font-size:14px;border-radius:16px;cursor:pointer;box-shadow:0 4px 24px rgba(26,107,60,.5);}'
      +'.ob-cta-ghost{width:100%;padding:13px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:rgba(240,237,230,.6);font-family:"Plus Jakarta Sans",sans-serif;font-weight:600;font-size:13px;border-radius:16px;cursor:pointer;}'
      +'.ob-emoji{font-size:56px;margin-bottom:16px;line-height:1;display:block;animation:ob-bounce .6s cubic-bezier(.34,1.56,.64,1) both;}'
      +'@keyframes ob-bounce{from{opacity:0;transform:scale(.6)}to{opacity:1;transform:scale(1)}}';
  ...
  pages.forEach(function(pg,i){
    ...
    orb.className='ob-bg-orb';
    page.appendChild(orb);
    if(pg.lottie){
      var lw=document.createElement('div');
      lw.className='ob-lottie-wrap';
      var lc=document.createElement('div');
      lc.className='ob-lottie-con';
      lc.id='ob-lot-'+i;
      lw.appendChild(lc);
      page.appendChild(lw);
    }
    ...
    if(!pg.isCta){
      ...
      con.innerHTML='<span class="ob-emoji">🚀</span>'
        +'<div class="ob-headline">'+pg.headline+'</div>'
        +'<div class="ob-sub">'+pg.sub+'</div>';
    }
  ...
  function goTo(idx){
    ...
    if(pages[cur].lottie) _initLot(cur);
  }

  function finish(){
    wrap.style.transition='opacity .3s ease';
    wrap.style.opacity='0';
    setTimeout(function(){wrap.remove();LOCAL.set('ob_done_'+ME.uid,'1');setTimeout(checkProfileComplete,600);},300);
  }
  ...
  function _initLot(idx){
    if(!window.lottie) return;
    var el=document.getElementById('ob-lot-'+idx);
    if(!el||el.dataset.li) return;
    el.dataset.li='1';
    window.lottie.loadAnimation({container:el,renderer:'svg',loop:true,autoplay:true,animationData:pages[idx].lottie()});
  }

  goTo(0);
  loadLottie(function(){_initLot(0);});
}

window.obNext=function(){};
window.obSkip=function(){
  var ov=document.getElementById('ob-cinematic')||document.getElementById('onboard-overlay');
  if(ov){ov.style.opacity='0';ov.style.transition='opacity .3s ease';setTimeout(function(){ov.remove();},300);}
  LOCAL.set('ob_done_'+ME.uid,'1');
