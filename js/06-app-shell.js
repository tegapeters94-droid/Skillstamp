...
window.showPage = function(name, skipHistory) {
  ...
  if (name === 'gigs') {
    loadGigsToCache().then(function() {
      ...
      // Always restore the gigs section visibility when navigating to page
      var gigsSection = document.querySelector('#page-gigs .section');
      if (gigsSection) gigsSection.style.display = '';

      // Clients: show only Proposals tab. Freelancers: show all 3 tabs.
      if (tabBar) tabBar.style.display = 'flex';
      var browseTab = document.getElementById('gtab-browse');
      if (browseTab) browseTab.style.display = isClient ? 'none' : '';
      ...
      if (_gigTab === 'myapps' && isFreelancer) renderMyApplications();
        ...
        // For clients, always render their gigs. For freelancers, renderGigs shows browse.
        if (isClient) {
          // Make sure browse tab is not marked active for clients
          var browseTabEl = document.getElementById('gtab-browse');
          if (browseTabEl) browseTabEl.classList.remove('active');
          var propTabEl = document.getElementById('gtab-proposals');
          if (propTabEl) propTabEl.classList.remove('active');
        }
        renderGigs();
      }
    });
