...
function showWelcomeModal(){
  ...
  var steps=isClient?[
    {icon:'💼',title:'Post a Gig',body:'Go to Gigs → tap "+ Post a Gig". Set a title, budget and deadline. Verified freelancers will apply with proposals.'},
    {icon:'👥',title:'Review & Hire',body:'When proposals arrive, tap "Review Applicants" to read cover notes, view portfolios and hire the best fit.'},
    {icon:'🔒',title:'Pay via Escrow',body:'Your budget is locked safely in escrow. Funds only release to the freelancer after you confirm the work is complete.'}
  ]:[
    {icon:'👤',title:'Complete Your Profile',body:'Add a photo, bio and skills so clients can trust you. A strong profile gets 3× more views than an empty one.'},
    {icon:'⚡',title:'Get Skill Verified',body:'Submit portfolio samples to get your SkillID badge. Verified freelancers get unlimited proposals and top placement.'},
    {icon:'💰',title:'Apply & Get Paid',body:'Browse Gigs, submit proposals, and get hired. Your wallet is credited the moment the client confirms delivery.'}
  ];
  var ov=document.createElement('div');
  ov.className='onboard-overlay';
  ov.id='onboard-overlay';
  var stepsHTML=steps.map(function(s,i){
    return '<div style="display:flex;align-items:flex-start;gap:12px;padding:12px;background:var(--s2);border-radius:10px;margin-bottom:8px;">'      +'<div style="width:36px;height:36px;border-radius:50%;background:var(--gld);color:#000;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">'+s.icon+'</div>'      +'<div style="flex:1;text-align:left;">'      +'<div style="font-family:Plus Jakarta Sans,sans-serif;font-weight:700;font-size:12px;margin-bottom:3px;color:var(--tx);">'+(i+1)+'. '+s.title+'</div>'      +'<div style="font-size:11px;color:var(--td);line-height:1.6;">'+s.body+'</div>'      +'</div></div>';
  }).join('');
  var ctaOnclick=isClient?"obSkip();openPostGig();":"obSkip();showPage('gigs');";
  var ctaLabel=isClient?'💼 Post Your First Gig →':'💼 Browse Open Gigs →';
  var secOnclick=isClient?"obSkip();showPage('talent');":"obSkip();showPage('myprofile');";
  var secLabel=isClient?'Browse Talent →':'Complete My Profile →';
  ov.innerHTML='<div class="onboard-sheet" style="max-height:92vh;overflow-y:auto;">'    +'<div style="font-size:44px;margin-bottom:10px;line-height:1;">'+(isClient?'🏢':'🌍')+'</div>'    +'<div style="font-family:Plus Jakarta Sans,sans-serif;font-weight:800;font-size:19px;margin-bottom:4px;color:var(--tx);">Welcome, '+firstName+'!</div>'    +'<div style="font-size:11px;color:var(--td);margin-bottom:18px;line-height:1.5;">Here\'s how SkillStamp works in 3 simple steps:</div>'    +stepsHTML    +'<button onclick="'+ctaOnclick+'" style="width:100%;padding:14px;background:var(--gld);color:#000;font-family:Plus Jakarta Sans,sans-serif;font-weight:800;font-size:13px;border:none;border-radius:12px;cursor:pointer;margin-top:14px;margin-bottom:8px;">'+ctaLabel+'</button>'    +'<button onclick="'+secOnclick+'" style="width:100%;padding:11px;background:none;border:1px solid var(--br);color:var(--tx);font-family:Plus Jakarta Sans,sans-serif;font-weight:600;font-size:12px;border-radius:12px;cursor:pointer;margin-bottom:6px;">'+secLabel+'</button>'    +'<button onclick="obSkip()" style="background:none;border:none;color:var(--td);font-size:11px;cursor:pointer;padding:6px;">Explore on my own</button>'    +'</div>';
  document.body.appendChild(ov);
}
window.obNext=function(){obSkip();};
