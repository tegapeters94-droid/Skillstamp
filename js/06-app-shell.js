...
function checkProfileComplete() {
  ...
  var doneCount = keys.filter(function(k) { return fields[k]; }).length;
  var pct = Math.round((doneCount / keys.length) * 100);

  if (pct >= 100) { setTimeout(maybeShowVerifNudge, 400); return; }

  // Mark shown for this session
  try { sessionStorage.setItem(sessKey, '1'); } catch(e) {}

  var tips = {
    bio:    { label: 'Add a bio',        desc: 'Let clients know who you are.' },
    skills: { label: 'Add your skills',  desc: 'Get discovered by the right clients.' },
    photo:  { label: 'Upload a photo',   desc: 'Profiles with photos earn 4x more trust.' },
    title:  { label: 'Set your title',   desc: 'Your professional headline — make it count.' }
  };

  var arcR = 26, arcC = 2 * Math.PI * arcR;

  var itemsHTML = keys.map(function(k) {
    var done = fields[k];
    var t = tips[k];
    var indicator = done
      ? '<svg width="22" height="22" viewBox="0 0 22 22"><circle cx="11" cy="11" r="10" fill="#059669"/><polyline points="6,11 9.5,14.5 16,8" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>'
      : '<div style="width:22px;height:22px;border-radius:50%;border:2px solid #d1d5db;"></div>';
    return '<div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid #f3f4f6;">'
      + '<div style="flex:1;">'
      + '<div style="font-family:-apple-system,BlinkMacSystemFont,Inter,sans-serif;font-weight:600;font-size:13px;color:' + (done ? '#9ca3af' : '#111827') + ';' + (done ? 'text-decoration:line-through;' : '') + '">' + t.label + '</div>'
  ...
  // Separate fixed overlay — NOT setModal
  if (document.getElementById('ss-pc-overlay')) return;

  // Inject keyframes once
  if (!document.getElementById('ss-pc-kf')) {
    var kf = document.createElement('style');
    kf.id = 'ss-pc-kf';
    kf.textContent = '@keyframes ss-fade-in{from{opacity:0}to{opacity:1}}@keyframes ss-slide-up{from{transform:translateY(100%)}to{transform:translateY(0)}}';
    document.head.appendChild(kf);
  }

  var overlay = document.createElement('div');
  overlay.id = 'ss-pc-overlay';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:99995;display:flex;align-items:flex-end;justify-content:center;background:rgba(0,0,0,.45);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);animation:ss-fade-in .2s ease;';

  var sheet = document.createElement('div');
  sheet.style.cssText = 'background:#fff;width:100%;max-width:480px;border-radius:20px 20px 0 0;padding:24px 20px 40px;box-sizing:border-box;max-height:90vh;overflow-y:auto;animation:ss-slide-up .3s cubic-bezier(.4,0,.2,1);';

  sheet.innerHTML =
    '<div style="width:36px;height:4px;background:#e5e7eb;border-radius:2px;margin:0 auto 20px;"></div>'
    + '<div style="background:linear-gradient(135deg,#ecfdf5,#f0fdf4);border-radius:16px;padding:16px;margin-bottom:20px;border:1px solid #a7f3d0;">'
    + '<div style="display:flex;align-items:center;gap:16px;">'
    + '<div style="position:relative;width:64px;height:64px;flex-shrink:0;">'
    + '<svg width="64" height="64" viewBox="0 0 64 64" style="transform:rotate(-90deg);">'
    + '<circle cx="32" cy="32" r="' + arcR + '" fill="none" stroke="#d1fae5" stroke-width="5"/>'
    + '<circle id="ss-pc-arc" cx="32" cy="32" r="' + arcR + '" fill="none" stroke="#059669" stroke-width="5" stroke-linecap="round" stroke-dasharray="0 ' + arcC + '" style="transition:stroke-dasharray .8s cubic-bezier(.34,1.1,.64,1);"/>'
    + '</svg>'
    + '<div id="ss-pc-pct" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:-apple-system,BlinkMacSystemFont,Inter,sans-serif;font-weight:800;font-size:14px;color:#111827;">0%</div>'
    + '</div>'
    + '<div style="flex:1;">'
    + '<div style="font-family:-apple-system,BlinkMacSystemFont,Inter,sans-serif;font-weight:800;font-size:16px;color:#111827;margin-bottom:4px;">Unlock Your Full Potential</div>'
    + '<div style="font-size:12px;color:#4b5563;line-height:1.5;">Complete profiles get <strong style="color:#059669;">3x more views</strong> from premium clients.</div>'
    + '</div></div></div>'
    + itemsHTML
    + '<div style="display:flex;gap:10px;margin-top:20px;">'
    + '<button id="ss-pc-go" style="flex:1;padding:14px;background:#059669;color:#fff;font-family:-apple-system,BlinkMacSystemFont,Inter,sans-serif;font-weight:700;font-size:14px;border:none;border-radius:12px;cursor:pointer;box-shadow:0 4px 14px rgba(5,150,105,.3);">Complete Profile</button>'
    + '<button id="ss-pc-later" style="padding:14px 18px;background:#f9fafb;border:1.5px solid #e5e7eb;color:#6b7280;font-family:-apple-system,BlinkMacSystemFont,Inter,sans-serif;font-weight:600;font-size:13px;border-radius:12px;cursor:pointer;">Later</button>'
    + '</div>';

  overlay.appendChild(sheet);
  document.body.appendChild(overlay);

  // Animate ring after two frames (ensures transition fires)
  requestAnimationFrame(function() {
    requestAnimationFrame(function() {
      var arc = document.getElementById('ss-pc-arc');
      var pctEl = document.getElementById('ss-pc-pct');
  ...
  overlay.addEventListener('click', function(e) { if (e.target === overlay) _closePC(); });

  setTimeout(function() {
    var go = document.getElementById('ss-pc-go');
    var later = document.getElementById('ss-pc-later');
    if (go) go.onclick = function() {
      _closePC();
      showPage('myprofile');
      setTimeout(function() { var eb = document.getElementById('edit-profile-btn'); if (eb) eb.click(); }, 400);
    };
    if (later) later.onclick = _closePC;
  }, 50);
}
...
function maybeShowVerifNudge(){
  ...
  var html=
    ...
    +(isInReview
      ?'<div style="font-family:Syne,sans-serif;font-weight:800;font-size:17px;margin-bottom:6px;color:var(--tx);">Verification in Progress</div>'
      ...
      :'<div style="font-family:Syne,sans-serif;font-weight:800;font-size:17px;margin-bottom:6px;color:var(--tx);">Unlock Your SkillID Badge</div>'
    ...
    +(isInReview?'':
      ...
      +[
        ...
        return '<div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px 10px;backdrop-filter:blur(4px);">'
          ...
          +'<div style="font-family:Syne,sans-serif;font-weight:700;font-size:11px;color:var(--tx);margin-bottom:2px;">'+b.label+'</div>'
    ...
    +(isInReview
      ?'<button id="vn-ok" style="flex:1;padding:13px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);color:var(--tx);font-family:Syne,sans-serif;font-weight:700;font-size:12px;border-radius:12px;cursor:pointer;">Got it — I\'ll wait</button>'
      :'<button id="vn-go" style="flex:1;padding:13px;background:linear-gradient(135deg,#1a6b3c,#0f4a28);color:#fff;font-family:Syne,sans-serif;font-weight:800;font-size:12px;border:none;border-radius:12px;cursor:pointer;letter-spacing:.01em;">Get Verified Now →</button>'
       +'<button id="vn-later" style="padding:13px 16px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:var(--td);font-family:Syne,sans-serif;font-weight:600;font-size:11px;border-radius:12px;cursor:pointer;">Not now</button>'
    )+'</div>';
...
window.openPortfolioItem = function(uid, itemId) {
  ...
  mh += '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;margin-bottom:4px;">' + item.title + '</div>';
...
// ── ONBOARDING ──────────────────────────────────────────────
// Single-card state machine — no scrolling, no track, no overflow

function showOnboarding() {
  if (document.getElementById('ss-ob-wrap')) return;
...
function _buildOnboarding() {
  ...
  // ── Inject styles once ────────────────────────────────
  if (!document.getElementById('ss-ob-style')) {
    ...
    sEl.textContent =
      '#ss-ob-wrap{'
        ...
        + 'animation:ss-ob-fadein .25s ease;'
      + '}'
      + '@keyframes ss-ob-fadein{from{opacity:0}to{opacity:1}}'
      + '#ss-ob-card{'
        + 'background:#fff;'
        + 'border-radius:24px;'
      ...
      + '#ss-ob-title{'
        ...
        + 'transition:opacity .15s ease;'
        + 'font-family:-apple-system,BlinkMacSystemFont,"Inter","Plus Jakarta Sans",sans-serif;'
      + '}'
      + '#ss-ob-sub{'
        + 'font-size:13px;font-weight:400;line-height:1.6;'
        + 'color:#4b5563;max-width:290px;'
        + 'transition:opacity .15s ease;'
        + 'font-family:-apple-system,BlinkMacSystemFont,"Inter","Plus Jakarta Sans",sans-serif;'
      + '}'
      + '#ss-ob-footer{'
      ...
      + '}'
      + '.ss-ob-dot{'
        + 'height:6px;border-radius:3px;background:#d1fae5;'
        + 'transition:all .3s cubic-bezier(.4,0,.2,1);width:6px;'
      + '}'
      + '.ss-ob-dot.on{width:20px;background:#059669;}'
      + '#ss-ob-btn-row{'
      ...
      + '}'
      + '#ss-ob-next:active{transform:scale(.96);box-shadow:0 2px 8px rgba(5,150,105,.2);}'
      + '#ss-ob-cta-col{'
      ...
      + '}'
      + '#ss-ob-cta-main:active{transform:scale(.98);}'
      + '#ss-ob-cta-sec{'
        + 'width:100%;padding:13px;'
        + 'background:#fff;border:1.5px solid #e5e7eb;'
        + 'color:#374151;'
        + 'font-family:-apple-system,BlinkMacSystemFont,"Inter",sans-serif;'
        + 'font-weight:600;font-size:14px;border-radius:14px;cursor:pointer;'
      + '}';
  ...
  // Anim zone
  var animZone = document.createElement('div');
  animZone.id = 'ss-ob-anim-zone';
  var fallback = document.createElement('div');
  fallback.id = 'ss-ob-fallback';
  fallback.textContent = slides[0].icon;
  animZone.appendChild(fallback);
  card.appendChild(animZone);

  // Text body
  var body = document.createElement('div');
  body.id = 'ss-ob-body';
  body.innerHTML =
    '<div id="ss-ob-tag">' + slides[0].tag + '</div>' +
    '<div id="ss-ob-title">' + slides[0].title + '</div>' +
    '<div id="ss-ob-sub">' + slides[0].sub + '</div>';
  card.appendChild(body);

  // Footer
  var footer = document.createElement('div');
  footer.id = 'ss-ob-footer';

  var dots = document.createElement('div');
  dots.id = 'ss-ob-dots';
  slides.forEach(function(_, i) {
    var d = document.createElement('div');
    d.className = 'ss-ob-dot' + (i === 0 ? ' on' : '');
    d.id = 'ss-ob-d' + i;
    dots.appendChild(d);
  });
  footer.appendChild(dots);

  // Nav row (skip + next) — shown on slides 0-3
  var btnRow = document.createElement('div');
  btnRow.id = 'ss-ob-btn-row';
  var skipBtn = document.createElement('button');
  skipBtn.id = 'ss-ob-skip';
  skipBtn.textContent = 'Skip';
  var nextBtn = document.createElement('button');
  nextBtn.id = 'ss-ob-next';
  nextBtn.textContent = 'Next →';
  btnRow.appendChild(skipBtn);
  btnRow.appendChild(nextBtn);
  footer.appendChild(btnRow);

  // CTA column (slide 4 only) — hidden initially
  var ctaCol = document.createElement('div');
  ctaCol.id = 'ss-ob-cta-col';
  ...
  ctaCol.appendChild(ctaMain);
  ctaCol.appendChild(ctaSec);
  footer.appendChild(ctaCol);

  card.appendChild(footer);
  wrap.appendChild(card);
  document.body.appendChild(wrap);

  // ── Lottie: Promise-based loader ─────────────────────
  // Resolves when lottie-player custom element is ready to use
  var _lottieReady = new Promise(function(resolve) {
    if (customElements.get('lottie-player')) { resolve(); return; }
    var sc = document.createElement('script');
    sc.src = 'https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js';
    sc.onload  = function() { setTimeout(resolve, 120); };
    sc.onerror = function() { resolve(); }; // resolve anyway — SVG fallback will show
    document.head.appendChild(sc);
  });

  // SVG fallbacks per slide (shown if Lottie fails/times out)
  var SVG_FALLBACKS = [
    // 0: Globe / network
    '<svg viewBox="0 0 80 80" width="80" height="80" fill="none" stroke="#059669" stroke-width="2"><circle cx="40" cy="40" r="30"/><ellipse cx="40" cy="40" rx="12" ry="30"/><line x1="10" y1="40" x2="70" y2="40"/><line x1="16" y1="24" x2="64" y2="24"/><line x1="16" y1="56" x2="64" y2="56"/></svg>',
    // 1: Target
    '<svg viewBox="0 0 80 80" width="80" height="80" fill="none" stroke="#059669" stroke-width="2"><circle cx="40" cy="40" r="30"/><circle cx="40" cy="40" r="18"/><circle cx="40" cy="40" r="6" fill="#059669"/><line x1="58" y1="22" x2="44" y2="36" stroke="#e8c547" stroke-width="2"/></svg>',
    // 2: Shield — Security slide
    '<svg viewBox="0 0 80 80" width="80" height="80" fill="none"><path d="M40 10 L65 22 L65 44 C65 58 40 70 40 70 C40 70 15 58 15 44 L15 22 Z" stroke="#059669" stroke-width="2.5" stroke-linejoin="round"/><polyline points="28,40 36,48 54,32" stroke="#059669" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>',
    // 3: Badge / medal
    '<svg viewBox="0 0 80 80" width="80" height="80" fill="none" stroke="#e8c547" stroke-width="2"><circle cx="40" cy="48" r="22"/><polygon points="40,28 43,36 52,36 45,41 48,50 40,45 32,50 35,41 28,36 37,36" fill="#e8c547" stroke="none"/><line x1="31" y1="16" x2="26" y2="28" stroke="#e8c547"/><line x1="49" y1="16" x2="54" y2="28" stroke="#e8c547"/></svg>',
    // 4: Checkmark / success
    '<svg viewBox="0 0 80 80" width="80" height="80" fill="none"><circle cx="40" cy="40" r="30" stroke="#059669" stroke-width="2.5"/><polyline points="24,40 34,52 56,28" stroke="#059669" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/></svg>'
  ];

  // Creates a fresh lottie-player for the given slide index
  // Returns a Promise that resolves when ready (or times out with SVG fallback)
  function _playSlide(idx) {
    // Remove any existing player
    var old = animZone.querySelector('lottie-player');
    if (old) old.remove();
    // Show fallback immediately
    fallback.style.opacity = '1';
    fallback.innerHTML = SVG_FALLBACKS[idx] || slides[idx].icon;

    return _lottieReady.then(function() {
      if (!customElements.get('lottie-player')) return; // library failed to load
      var p = document.createElement('lottie-player');
      p.setAttribute('src', slides[idx].src);
      p.setAttribute('background', 'transparent');
      p.setAttribute('speed', '1');
      p.setAttribute('loop', '');
      p.setAttribute('autoplay', '');
      p.style.cssText = 'width:200px;height:200px;position:absolute;';
      animZone.appendChild(p);

      // Hide SVG fallback once lottie is ready
      var hideTimer = setTimeout(function() {
        fallback.style.opacity = '0';
      }, 2000); // 2s max wait before hiding anyway
  ...
  function _render(idx) {
    ...
    var tagEl   = document.getElementById('ss-ob-tag');
    var titleEl = document.getElementById('ss-ob-title');
    var subEl   = document.getElementById('ss-ob-sub');
    [tagEl, titleEl, subEl].forEach(function(el) { if (el) el.style.opacity = '0'; });

    setTimeout(function() {
      if (tagEl)   tagEl.textContent   = sl.tag;
      if (titleEl) titleEl.textContent = sl.title;
      if (subEl)   subEl.textContent   = sl.sub;
      [tagEl, titleEl, subEl].forEach(function(el) { if (el) el.style.opacity = '1'; });

      // Show/hide nav vs CTA
      btnRow.style.display = sl.isCta ? 'none' : 'flex';
      ctaCol.style.display = sl.isCta ? 'flex' : 'none';
    }, 150);

    // Update dots
    slides.forEach(function(_, i) {
      var d = document.getElementById('ss-ob-d' + i);
      if (d) d.classList.toggle('on', i === idx);
    });

    // Recreate lottie player for this slide — fixes Security animation glitch
    _playSlide(idx);
  }
  ...
  function _finish() {
    ...
    setTimeout(function() {
      wrap.remove();
      LOCAL.set('ob_done_' + ME.uid, '1');
      setTimeout(checkProfileComplete, 400);
    }, 250);
  }
  ...
  nextBtn.onclick = function() {
    if (cur < slides.length - 1) { cur++; _render(cur); }
    else { _finish(); }
  };

  ctaMain.onclick = function() {
    _finish();
    setTimeout(function() {
      if (isClient && typeof openPostGig === 'function') openPostGig();
      else showPage('gigs');
    }, 300);
  };

  ctaSec.onclick = function() {
    _finish();
    setTimeout(function() { showPage(isClient ? 'talent' : 'myprofile'); }, 300);
  };
  ...
  wrap.addEventListener('touchend', function(e) {
    ...
    if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 44) {
      if (dx < 0 && cur < slides.length - 1) { cur++; _render(cur); }
      else if (dx > 0 && cur > 0)            { cur--; _render(cur); }
    }
  }, { passive: true });
}

window.obSkip = function() {
  var el = document.getElementById('ss-ob-wrap')
        || document.getElementById('ss-ob')
        || document.getElementById('ob-cinematic')
...
window.switchGigTab=function(tab){
  ...
  var myappsTab=document.getElementById("gtab-myapps");
  if(myappsTab) myappsTab.classList.toggle("active",tab==="myapps");
  var propTab=document.getElementById("gtab-proposals");
  if(propTab) propTab.classList.toggle("active",tab==="proposals");
  var ca=document.getElementById("gig-content-area");
  if(ca) ca.innerHTML='';
  if(tab==="browse")         renderGigs();
  else if(tab==="myapps")    renderMyApplications();
  else if(tab==="proposals") renderProposalHub();
};
...
function renderMyApplications(){
  ...
  if(!apps.length){
    content.innerHTML='<div style="padding:40px 20px;text-align:center;">'
      ...
      +'<div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;margin-bottom:6px;">No applications yet</div>'
  ...
  h+='<div style="font-family:Syne,sans-serif;font-weight:700;font-size:13px;margin-bottom:14px;">My Applications ('+apps.length+')</div>';
  apps.forEach(function(app){
    ...
    h+='<div style="background:var(--s);border:1px solid var(--br);border-radius:12px;padding:16px;margin-bottom:12px;">'
      ...
      +'<div style="font-family:Syne,sans-serif;font-weight:700;font-size:13px;margin-bottom:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+app.title+'</div>'
      +'<div style="font-size:10px;color:var(--td);">'+( poster?poster.name:"Client")+' · Applied '+timeAgo(app.appliedAt)+'</div>'
      +'</div>'
      +'<span style="flex-shrink:0;background:'+cfg.bg+';border:1px solid '+cfg.border+';color:'+cfg.color+';font-size:9px;font-family:Syne,sans-serif;font-weight:700;padding:3px 9px;border-radius:20px;white-space:nowrap;">'+cfg.icon+' '+cfg.label+'</span>'
      +'</div>'
      ...
      +(gig&&(gig.status==="hired"||gig.status==="completed")&&gig.hiredUid===ME.uid?
        '<button onclick="openGigWorkspace(\''+app.gigId+'\')" style="background:var(--gld);color:#000;font-family:Syne,sans-serif;font-weight:700;font-size:10px;padding:6px 14px;border-radius:6px;cursor:pointer;margin-right:6px;">💼 Workspace</button>'
        :'')
...
window.openDeliveryForm=function(gid){
  ...
  mh+='<div style="font-family:Syne,sans-serif;font-weight:800;font-size:15px;margin-bottom:4px;">\uD83D\uDCE6 Submit Delivery</div>';
...
window.openRevisionRequest=function(gid){
  ...
  mh+='<div style="font-family:Syne,sans-serif;font-weight:800;font-size:15px;margin-bottom:4px;">\uD83D\uDD04 Request Revision</div>';
...
window.openRatingPrompt=function(gid,isClient){
  ...
  mh+='<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;margin-bottom:6px;">Rate '+other.name+'</div>';
...
window.openDirectProposal = function(uid) {
  ...
  if (alreadyHired) {
    setModal(
      ...
      '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;margin-bottom:8px;">Already Working Together</div>' +
  ...
  var avEl = u.avatar
    ...
    : '<div style="width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,' + u.gradient + ',' + u.gradient + '88);display:flex;align-items:center;justify-content:center;font-family:Syne,sans-serif;font-weight:800;font-size:19px;color:#000;border:2px solid var(--gld);">' + initials(u.name) + '</div>';
  ...
  panel.innerHTML =
    ...
    '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;color:var(--tx);">Direct Proposal</div>' +
    '</div>' +
    ...
    avEl +
    '<div style="flex:1;min-width:0;">' +
    '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:14px;color:var(--tx);margin-bottom:3px;">' + u.name + '</div>' +
    '<div style="font-size:10px;color:var(--td);margin-bottom:5px;">' + (u.title || 'SkillStamp Freelancer') + '</div>' +
    verifBadge +
    '</div>' +
    (u.score > 0 ? '<div style="text-align:center;flex-shrink:0;"><div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;color:var(--gld);">' + u.score.toFixed(1) + '</div><div style="font-size:8px;color:var(--td);">Rating</div></div>' : '') +
    '</div>' +
    ...
    (isVerif ? '<div style="background:var(--s2);border:1px solid var(--br);border-radius:10px;padding:10px 13px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center;"><div style="font-size:11px;color:var(--td);">Your wallet balance</div><div style="font-family:Syne,sans-serif;font-weight:800;font-size:15px;color:var(--grn);">$' + Math.round((ME.wallet && ME.wallet.balance) || 0).toLocaleString() + '</div></div>' : '') +
  ...
  if (submitBtn) submitBtn.onclick = async function() {
    ...
    var budget = 0;
    if (isVerif) {
      budget = parseFloat(document.getElementById('dp-budget').value) || 0;
      if (!budget || budget < 1) { toast('Please enter a valid budget.', 'bad'); return; }
      ...
      budget = budgetRange; // string range for unverified
    }
    ...
    try {
      ...
      // Deduct escrow if verified
      if (isVerif) {
        ME.wallet.balance  = Math.max(0, (ME.wallet.balance || 0) - budget);
        ME.wallet.pending  = (ME.wallet.pending || 0) + budget;
        ME.wallet.transactions.unshift({ id: 't' + Date.now(), type: 'out', amount: budget, from: ME.name, desc: 'Escrow: ' + title, ts: Date.now() });
        await saveUser(ME);
        renderWallet();
      }
      ...
      // Only show error if proposal was NOT saved — check by err type
      var panelStillOpen = !!document.getElementById('proposal-panel');
      if (panelStillOpen) {
        submitBtn.disabled = false;
        submitBtn.textContent = isVerif ? '💼 Send Proposal & Lock Escrow →' : '📩 Send Direct Enquiry →';
        toast('Could not send proposal. Check your connection and try again.', 'bad');
      }
      // If panel was already removed = proposal succeeded, secondary action failed — do nothing
    }
  };
};
...
window.loadVerifQueue = async function() {
  ...
  try {
    ...
    container.innerHTML = '<div style="font-family:Syne,sans-serif;font-weight:700;font-size:13px;margin-bottom:12px;">Verification Queue (' + pending.length + ')</div>';
    ...
    pending.forEach(function(v) {
      ...
      var av = v.avatar ? '<img src="'+v.avatar+'" style="width:38px;height:38px;border-radius:50%;object-fit:cover;">' : '<div style="width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,'+(v.gradient||'#888')+','+(v.gradient||'#888')+'88);display:flex;align-items:center;justify-content:center;font-family:Syne,sans-serif;font-weight:700;font-size:13px;color:#000;">'+initials(v.name||'?')+'</div>';
      card += av;
      card += '<div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:13px;">' + (v.name||'Unknown') + '</div>';
...
// ── SPA Routing ───────────────────────────────────────────────
// Strategy: always keep a sentinel history entry so the browser
// never exits the app on back. We push app pages ON TOP of that.
window._pageHistory  = [];
window._currentPage  = 'home';
window._spaReady     = false;

// Called once after login — plants the sentinel
function _initSPAHistory() {
  if (window._spaReady) return;
  window._spaReady = true;
  // Replace current entry with sentinel (no page data — acts as exit guard)
  try { history.replaceState({ _sentinel: true }, '', location.pathname + location.search); } catch(e) {}
  // Immediately push the home entry on top
...
window.showPage = function(name, skipHistory) {
  ...
  if (!skipHistory && window._currentPage && window._currentPage !== name) {
    ...
    if (window._pageHistory.length > 30) window._pageHistory.shift();
  }
  window._currentPage = name;
  try { sessionStorage.setItem('ss_page', name); localStorage.setItem('ss_last_page', name); } catch(e) {}
  // Only push a new history entry (don't push when called from popstate)
  if (!skipHistory) {
  ...
  if (name === 'gigs') {
    loadGigsToCache().then(function() {
      var isFreelancer = ME && ME.role === 'freelancer';
      var tabBar = document.getElementById('gig-tabs-bar');
      if (tabBar) tabBar.style.display = 'flex';

      // My Applications tab — freelancers only
      var myappsTab = document.getElementById('gtab-myapps');
      if (myappsTab) myappsTab.style.display = (ME && ME.role === 'freelancer') ? '' : 'none';

      // Update applications badge (freelancers)
      ...
      // Update proposals badge (async — check pending proposals)
      fbGetAll('proposals').then(function(all) {
        var mine = all.filter(function(p) {
          return p.toUid === ME.uid && p.status === 'pending';
        });
        var pc = document.getElementById('proposals-count');
        if (pc) { pc.textContent = mine.length; pc.style.display = mine.length ? '' : 'none'; }
      }).catch(function() {});

      if (_gigTab === 'myapps') renderMyApplications();
      else if (_gigTab === 'proposals') renderProposalHub();
      else renderGigs();
    });
  }
...
window.addEventListener('popstate', function(e) {
  ...
  // We hit the sentinel entry — user wants to "leave"
  // Instead of leaving, push back home and stay
  if (e.state && e.state._sentinel) {
    try { history.pushState({ page: 'home' }, '', '#home'); } catch(ex) {}
    showPage('home', true);
    return;
  }

  // Normal internal navigation — pop our own stack
...
// Wire enterApp to init SPA history
var _origEnterApp = window.enterApp || function(){};
// Patch: _initSPAHistory is called on first showPage call above,
// but we also call it explicitly from enterApp hook below.
var _enterAppPatched = false;
function _patchEnterAppSPA() {
  if (_enterAppPatched) return;
  _enterAppPatched = true;
  var _ea = window.enterApp;
  if (_ea) {
    window.enterApp = function() {
      _ea.apply(this, arguments);
      setTimeout(_initSPAHistory, 100);
    };
  }
}
_patchEnterAppSPA();
