...
window.openGigWorkspace=async function(gid){
  ...
  setTimeout(function(){
    ...
    function _doOpenChat(e) {
      ...
      // Call openMsg directly — it handles its own loading screen and fullscreen setup
      openMsg(_chatUid);
    }
...
window.showPage = function(name, skipHistory) {
  ...
  if (name === 'talent') {
    if (ME && (!CACHE.users || CACHE.users.length === 0)) {
      ...
      fbGetAll('users').then(function(users) {
        if (users && users.length) CACHE.users = users;
        renderTalent();
      }).catch(function() { renderTalent(); });
    } else {
      renderTalent();
    }
  }
...
window.addEventListener('popstate', function(e) {
  // Ignore pops triggered by our own pushState calls or settings panel removal
  if (window._ignorePop) {
    window._ignorePop = false;
    return;
  }

  // Settings panel open — just close it, don't navigate
  ...
  // Sentinel entry — stay in app, go home
  ...
  // Normal back navigation — pop our internal stack
