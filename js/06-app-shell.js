...
window.openGigWorkspace=async function(gid){
  ...
  setTimeout(function(){
    ...
    setTimeout(function(){
      var mb=document.getElementById('ws-msg');
      if(mb) mb.onclick=function(){
        var chatUid=isClient?gig.hiredUid:gig.posterUid;
        // Reset mcontent style before switching to full-screen chat modal
        var mc=document.getElementById('mcontent');
        var ov=document.getElementById('moverlay');
        if(mc) mc.style.cssText='';
        if(ov) ov.style.cssText='';
        closeModal();
        setTimeout(function(){ openMsg(chatUid); },150);
      };
    },50);
  },50);
};
