...
function _buildOnboarding() {
  ...
  // ── Lottie player (single instance, src swapped) ──────
  var player = null;
  ...
  function _ensureLottie() {
    ...
    player.setAttribute('src', slides[0].src);
    player.setAttribute('background', 'transparent');
    player.setAttribute('speed', '1');
    player.setAttribute('loop', '');
    player.setAttribute('autoplay', '');
    animZone.appendChild(player);
    player.addEventListener('ready', function() {
      fallback.style.opacity = '0';
    }, { once: true });
    setTimeout(function() { fallback.style.opacity = '0'; }, 1800);
  }
  ...
  if (!customElements.get('lottie-player')) {
    ...
    sc.onload = function() { setTimeout(_ensureLottie, 100); };
  ...
  function _render(idx, dir) {
    ...
    var tag   = document.getElementById('ss-ob-tag');
    var title = document.getElementById('ss-ob-title');
    var sub   = document.getElementById('ss-ob-sub');
    [tag, title, sub].forEach(function(el) { if (el) el.style.opacity = '0'; });
    fallback.style.opacity = '0';

    setTimeout(function() {
      if (tag)   tag.textContent   = sl.tag;
      if (title) title.textContent = sl.title;
      if (sub)   sub.textContent   = sl.sub;
      fallback.textContent = sl.icon;

      // Swap lottie src
      if (player) {
        try { player.load(sl.src); } catch(e) {
          player.setAttribute('src', sl.src);
        }
        fallback.style.opacity = '0';
        // Brief fallback show while new animation loads
        setTimeout(function() { fallback.style.opacity = '0'; }, 800);
      } else {
        fallback.style.opacity = '1';
      }

      [tag, title, sub].forEach(function(el) { if (el) el.style.opacity = '1'; });

      // Show/hide nav vs CTA
      var isCta = !!sl.isCta;
      btnRow.style.display  = isCta ? 'none' : 'flex';
      ctaCol.style.display  = isCta ? 'flex' : 'none';
    }, 150);
  ...
  wrap.addEventListener('touchend', function(e) {
    ...
    if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 44) {
      if (dx < 0 && cur < slides.length - 1) { cur++; _render(cur, 1); }
      else if (dx > 0 && cur > 0)            { cur--; _render(cur, -1); }
    }
  }, { passive: true });
}
