...
// ── ONBOARDING ──────────────────────────────────────────────
// Clean, mobile-first 5-slide light-mode slider with lottie-player

function _ssLoadLottiePlayer(cb) {
  if (customElements.get('lottie-player')) { cb(); return; }
  var s = document.createElement('script');
  s.src = 'https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js';
  s.onload = function () { setTimeout(cb, 120); };
  s.onerror = cb; // graceful — emojis show if CDN fails
  document.head.appendChild(s);
}

// Public LottieFiles JSON CDN URLs
var _SS_LOTTIES = [
  'https://assets10.lottiefiles.com/packages/lf20_uwWgICKCxj.json',
  'https://assets9.lottiefiles.com/packages/lf20_qp1q7mct.json',
  'https://assets5.lottiefiles.com/packages/lf20_xvmprfgt.json',
  'https://assets6.lottiefiles.com/packages/lf20_jcikwtux.json',
  'https://assets2.lottiefiles.com/packages/lf20_jR229r.json'
];
var _SS_FALLBACK_ICONS = ['🌐','🎯','🛡️','🏅','✅'];

function showOnboarding() {
  if (document.getElementById('ss-ob')) return;
...
function _buildOnboarding() {
  ...
  var slides = [
    {
      ...
      sub: 'SkillStamp connects Africa\'s elite digital talent with global opportunities through a high-trust, verified ecosystem.',
      li: 0
    },
    {
      tag: 'Smart Matching',
      title: 'Precision\nMatching.',
      sub: 'No more bidding wars. Our engine pairs your exact skills to the right gigs — instantly and intelligently.',
      li: 1
    },
    {
      tag: 'Security',
      title: 'Work with\nPeace of Mind.',
      sub: 'Your payments are protected. Our Communication Guard and Escrow system ensure you get paid for every milestone.',
      li: 2
    },
    {
      tag: 'Verification',
    ...
    },
    {
      ...
      sub: 'Join thousands of verified African professionals. Your first gig is waiting.',
      li: 4,
      isCta: true
    }
  ];
  ...
  if (!document.getElementById('ss-ob-css')) {
    ...
    css.textContent = [
      ...
      '}',
      '.ss-ob-anim lottie-player{',
        'width:220px;',
        'height:220px;',
      '}',
      '.ss-ob-anim .ss-ob-icon{',
      ...
      '}',
      '.ss-ob-next:active{transform:scale(.97);box-shadow:0 2px 8px rgba(5,150,105,.25);}',
      '.ss-ob-btn-main{',
      ...
      '}',
      '.ss-ob-btn-main:active{transform:scale(.98);}',
      '.ss-ob-btn-ghost{',
  ...
  viewport.id = 'ss-ob-viewport';

  var track = document.createElement('div');
  track.id = 'ss-ob-track';

  slides.forEach(function (sl, i) {
    var slide = document.createElement('div');
    slide.className = 'ss-ob-slide';

    // Animation zone
    var animZone = document.createElement('div');
    animZone.className = 'ss-ob-anim';
    animZone.id = 'ss-ob-az-' + i;
    // Fallback icon shown until lottie loads
    var icon = document.createElement('div');
    icon.className = 'ss-ob-icon';
    icon.textContent = _SS_FALLBACK_ICONS[sl.li];
    animZone.appendChild(icon);
    slide.appendChild(animZone);

    // Text body
    var body = document.createElement('div');
    body.className = 'ss-ob-body';
    body.innerHTML =
      '<div class="ss-ob-tag">' + sl.tag + '</div>' +
      '<div class="ss-ob-title">' + sl.title + '</div>' +
      '<div class="ss-ob-sub">' + sl.sub + '</div>';
    slide.appendChild(body);

    // Footer
    var footer = document.createElement('div');
    footer.className = 'ss-ob-footer';

    // Dots
    var dotsRow = document.createElement('div');
    dotsRow.className = 'ss-ob-dots';
    dotsRow.id = 'ss-ob-dots-' + i;
    slides.forEach(function (_, di) {
      var d = document.createElement('div');
      d.className = 'ss-ob-dot' + (di === i ? ' on' : '');
    ...
    if (!sl.isCta) {
      ...
      row.className = 'ss-ob-row';
      var skipBtn = document.createElement('button');
      skipBtn.className = 'ss-ob-skip';
      skipBtn.textContent = 'Skip';
      skipBtn.id = 'ss-ob-skip-' + i;
      var nextBtn = document.createElement('button');
      nextBtn.className = 'ss-ob-next';
      nextBtn.textContent = 'Next →';
      nextBtn.id = 'ss-ob-next-' + i;
      row.appendChild(skipBtn);
      row.appendChild(nextBtn);
      footer.appendChild(row);
    } else {
      var btnMain = document.createElement('button');
      btnMain.className = 'ss-ob-btn-main';
      btnMain.id = 'ss-ob-cta-main';
      btnMain.textContent = isClient ? '💼 Post Your First Gig' : '🔍 Browse Open Gigs';
      var btnGhost = document.createElement('button');
      btnGhost.className = 'ss-ob-btn-ghost';
      btnGhost.id = 'ss-ob-cta-sec';
      btnGhost.textContent = isClient ? 'Browse Talent' : 'Complete My Profile';
      footer.appendChild(btnMain);
      footer.appendChild(btnGhost);
    }
  ...
  function goTo(idx) {
    ...
    // Update every slide's dot set
    for (var s = 0; s < n; s++) {
      var dr = document.getElementById('ss-ob-dots-' + s);
      if (!dr) continue;
      var dots = dr.querySelectorAll('.ss-ob-dot');
  ...
  function finish() {
    ...
    setTimeout(function () {
      wrap.remove();
      LOCAL.set('ob_done_' + ME.uid, '1');
      // Modal fires AFTER onboarding is fully removed
      setTimeout(checkProfileComplete, 400);
    }, 250);
  }

  // Wire skip / next buttons
  for (var i = 0; i < n; i++) {
    (function (idx) {
      var sk = document.getElementById('ss-ob-skip-' + idx);
      if (sk) sk.onclick = finish;
      var nx = document.getElementById('ss-ob-next-' + idx);
      if (nx) nx.onclick = function () { if (cur < n - 1) goTo(cur + 1); else finish(); };
    })(i);
  }

  // CTA slide buttons
  var ctaMain = document.getElementById('ss-ob-cta-main');
  ...
  var ctaSec = document.getElementById('ss-ob-cta-sec');
  if (ctaSec) ctaSec.onclick = function () {
    finish();
    setTimeout(function () {
      showPage(isClient ? 'talent' : 'myprofile');
    }, 300);
  };
  ...
  // ── Lottie init (lazy per slide) ───────────────────────
  function _tryInitLottie(idx) {
    if (lottiesLoaded[idx]) return;
    if (!customElements.get('lottie-player')) return;
    lottiesLoaded[idx] = true;
    var az = document.getElementById('ss-ob-az-' + idx);
    if (!az) return;
    var icon = az.querySelector('.ss-ob-icon');
    var player = document.createElement('lottie-player');
    player.setAttribute('src', _SS_LOTTIES[slides[idx].li]);
    ...
    // Hide fallback icon when lottie is ready
  ...
  // Kick off
  goTo(0);
  _ssLoadLottiePlayer(function () {
    _tryInitLottie(0);
  });
}

window.obSkip = function () {
  var el = document.getElementById('ss-ob') || document.getElementById('ob-cinematic') || document.getElementById('onboard-overlay');
  if (el) { el.style.transition = 'opacity .25s ease'; el.style.opacity = '0'; setTimeout(function () { el.remove(); }, 250); }
  LOCAL.set('ob_done_' + ME.uid, '1');
  setTimeout(checkProfileComplete, 400);
};
