...
window.renderTalent = function() {
  ...
  if (!users.length) {
    grid.innerHTML = '<div style="padding:48px 16px;text-align:center;"><div style="font-size:40px;margin-bottom:12px;">🔍</div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;margin-bottom:6px;">No talent found</div><div style="font-size:12px;color:var(--td);">Try a different category or search term</div></div>';
    return;
  }
...
function talentCard(u) {
  ...
  var av = u.avatar
    ...
    : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-family:Syne,sans-serif;font-weight:800;font-size:22px;color:#000;">'+initials(u.name)+'</div>';
  ...
  var chainHtml = isVerif && u.skillId
    ? '<div style="display:flex;align-items:center;gap:6px;padding:6px 10px;background:rgba(96,165,250,.05);border:1px solid rgba(96,165,250,.15);border-radius:7px;margin-bottom:12px;">'
      ...
      +'<span style="font-size:9px;color:#60a5fa;font-family:Syne,sans-serif;font-weight:700;letter-spacing:.06em;">'+u.skillId+'</span>'
      +'</div>'
    : '';

  // YOU badge
  var youBadge = isMe ? '<span style="font-size:8px;background:rgba(232,197,71,.15);border:1px solid rgba(232,197,71,.3);color:var(--gld);padding:2px 6px;border-radius:6px;font-weight:700;font-family:Syne,sans-serif;">YOU</span>' : '';

  // Verified badge
  var verifBadge = isVerif
    ? '<span style="display:inline-flex;align-items:center;gap:2px;background:rgba(74,222,128,.1);border:1px solid rgba(74,222,128,.25);color:#4ade80;font-size:8px;font-weight:700;padding:2px 7px;border-radius:20px;font-family:Syne,sans-serif;">'+verifiedSVG(getVerifColor())+' Verified</span>'
    : '';
  ...
  return `<div class="tc-card" onclick="viewProfile('${u.uid}')" style="cursor:pointer;">
    ...
    <div class="tc-top">
      ...
      <div class="tc-info">
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:3px;">
          <div style="font-family:Syne,sans-serif;font-weight:800;font-size:14px;color:var(--tx);line-height:1.2;">${u.name}</div>
          ${youBadge}
        </div>
    ...
    <div style="display:flex;align-items:center;gap:6px;margin-bottom:10px;">
      <span style="font-size:10px;background:var(--s2);border:1px solid var(--br);color:var(--td);padding:3px 9px;border-radius:20px;font-family:Syne,sans-serif;">${CAT_ICONS[u.category]||''} ${u.category}</span>
    </div>
    ...
    <div class="tc-stats">
      <div class="tc-stat">
        ...
        <div style="font-family:Syne,sans-serif;font-weight:800;font-size:13px;color:${rating?'var(--gld)':'var(--td)'};">${rating||'New'}</div>
        <div style="font-size:8px;color:var(--td);text-transform:uppercase;letter-spacing:.05em;">Rating</div>
      </div>
      <div class="tc-stat-div"></div>
      <div class="tc-stat">
        <div style="font-family:Syne,sans-serif;font-weight:800;font-size:13px;color:var(--tx);">${u.gigsCount||0}</div>
        <div style="font-size:8px;color:var(--td);text-transform:uppercase;letter-spacing:.05em;">Gigs</div>
      </div>
      <div class="tc-stat-div"></div>
      <div class="tc-stat">
        <div style="font-family:Syne,sans-serif;font-weight:800;font-size:13px;color:var(--tx);">${endorse.length}</div>
