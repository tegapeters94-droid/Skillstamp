...
function talentCard(u){
  ...
  return `<div class="card card-accent" onclick="viewProfile('${u.uid}')" style="cursor:pointer;">
    ...
    ${!isMe?'<div style="display:flex;gap:6px;margin-top:10px;"><button class="bgrn" onclick="event.stopPropagation();openEndorse(\''+u.uid+'\')" >🤝 Endorse</button><button class="b2sm" onclick="event.stopPropagation();viewProfile(\''+u.uid+'\')" >Profile</button></div>':''}
  </div>`;
}
