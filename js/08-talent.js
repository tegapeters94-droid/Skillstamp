...
window.renderTalent = function() {
  ...
  // Smart-Sort: Verified+Boosted (4) > Verified (3) > Unverified+Boosted (2) > Standard (1)
  // Within each tier: Pro users rank above free, then by repPoints
  users = users.sort((a, b) => {
    var aV = userIsVerified(a) ? 1 : 0;
    var bV = userIsVerified(b) ? 1 : 0;
    var aBoosted = a.profileBoostedUntil && a.profileBoostedUntil > Date.now() ? 1 : 0;
    var bBoosted = b.profileBoostedUntil && b.profileBoostedUntil > Date.now() ? 1 : 0;
    var aPro = userIsPro(a) ? 1 : 0;
    var bPro = userIsPro(b) ? 1 : 0;
    function smartScore(v, boosted, pro) {
      if (v && boosted) return 40 + pro * 5;
      if (v) return 30 + pro * 5;
      if (boosted) return 20 + pro * 5;
      return 10 + pro * 5;
    }
    var diff = smartScore(bV, bBoosted, bPro) - smartScore(aV, aBoosted, aPro);
...
function talentCard(u) {
  ...
  const isVerif = userIsVerified(u);
  const isPro = userIsPro(u);
  const isBoosted = u.profileBoostedUntil && u.profileBoostedUntil > Date.now();
  ...
  var proBadge = isPro ? '<span style="font-size:7px;font-weight:800;font-family:Plus Jakarta Sans,sans-serif;background:rgba(232,197,71,.2);color:var(--gld);border:1px solid rgba(232,197,71,.4);padding:1px 5px;border-radius:6px;margin-left:3px;">PRO</span>' : '';
  var boostedBorder = isBoosted ? 'box-shadow:0 0 0 2px rgba(96,165,250,.4);' : '';
  // Avatar
  var av = u.avatar
