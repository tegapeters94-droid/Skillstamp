...
window.runGlobalSearch=function(q){
  ...
  if(matchedUsers.length){
    ...
    matchedUsers.forEach(function(u){
      ...
      var avStyle='width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,'+u.gradient+','+u.gradient+'99);display:flex;align-items:center;justify-content:center;font-family:Syne,sans-serif;font-weight:800;font-size:14px;color:#000;flex-shrink:0;overflow:hidden;';
  ...
  if(!matchedUsers.length&&!matchedGigs.length){
    html='<div class="search-empty"><div style="font-size:40px;margin-bottom:12px;opacity:.4;">🤷</div><div style="font-family:Syne,sans-serif;font-weight:700;font-size:14px;margin-bottom:6px;">No results for "'+q+'"</div><div style="font-size:12px;">Try a skill name, category, country, or gig title</div></div>';
  }
