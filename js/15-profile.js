...
function buildProfile(u,isOwn){
  ...
  if(isOwn&&!isClient&&isVerifiedBadge&&u.skillId){
    ...
    for(var ii=0;ii<infoRows.length;ii++){
      infoHtml+='<div style="text-align:center;"><div style="font-size:8px;color:rgba(255,255,255,.55);text-transform:uppercase;letter-spacing:.06em;margin-bottom:2px;">'+infoRows[ii][0]+'</div><div style="font-size:10px;font-family:Syne,sans-serif;font-weight:600;color:#fff;">'+infoRows[ii][1]+'</div></div>';
    }
  ...
  if(isOwn){
    if(isClient){
      ...
      editBtns=(!alreadyVerified&&!pendingVerif
        ?'<div style="display:flex;gap:7px;margin-top:13px;"><button class="b2sm" onclick="openSubmitSkill()">⚡ Get Verified</button></div>'
        :'');
  ...
  if(endorsements.length){
    endorsements.slice(0,5).forEach(function(en){
      ...
      endorseItemsHtml+='<div class="endorse-item">'
        ...
        +'<div style="flex:1;"><div style="font-family:Syne,sans-serif;font-weight:600;font-size:10px;">'+en.fromName+'</div>'
  ...
  if(!isOwn){
    ...
    actionBtns+='<button onclick="openReportProfile(\'' + u.uid + '\',\'' + encodeURIComponent(u.name) + '\')" style="width:100%;margin-top:6px;background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.18);color:#ef4444;font-family:Syne,sans-serif;font-size:10px;font-weight:700;padding:8px 12px;border-radius:8px;cursor:pointer;">🚩 Report Profile</button>';
  }
  ...
  if(isClient){
    ...
    var _earned = ((u.wallet && u.wallet.earned) || u.earned || 0);
    var _earnedStr = _earned >= 1000 ? ('$' + (_earned/1000).toFixed(1) + 'k') : ('$' + _earned.toLocaleString());

    // SVG line icons
    var _starSVG = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#b45309" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>';
    var _briefSVG = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg>';
    var _coinSVG  = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#059669" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M14.8 9A2 2 0 0 0 12 7h-1a2 2 0 0 0 0 4h2a2 2 0 0 1 0 4h-1a2 2 0 0 1-2.8-1"/><line x1="12" y1="6" x2="12" y2="7"/><line x1="12" y1="17" x2="12" y2="18"/></svg>';

    statsRow = '<div class="pstatbar">'
      + '<div class="pstatbar-item">'
      + '<div class="pstatbar-icon">' + _starSVG + '</div>'
      + '<div class="pstatbar-val">' + _rt + '</div>'
      + '<div class="pstatbar-lbl">Rating</div>'
      + '</div>'
      + '<div class="pstatbar-div"></div>'
      + '<div class="pstatbar-item">'
      + '<div class="pstatbar-icon">' + _briefSVG + '</div>'
      + '<div class="pstatbar-val">' + _gc + '</div>'
      + '<div class="pstatbar-lbl">Gigs</div>'
      + '</div>'
      + '<div class="pstatbar-div"></div>'
      + '<div class="pstatbar-item">'
      + '<div class="pstatbar-icon">' + _coinSVG + '</div>'
      + '<div class="pstatbar-val">' + _earnedStr + '</div>'
      + '<div class="pstatbar-lbl">Earned</div>'
      + '</div>'
      + '</div>';
  }
...
window.openSubmitSkill = function() {
  ...
  if (ME.verifBannedUntil && Date.now() < ME.verifBannedUntil) {
    ...
    setModal('<button class="mclose" onclick="closeModal()">&#x2715;</button>'
      ...
      + '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;margin-bottom:8px;">Come Back Later</div>'
  ...
  if (ME.verifPermanentBan) {
    setModal('<button class="mclose" onclick="closeModal()">&#x2715;</button>'
      + '<div style="text-align:center;padding:20px 0;">'
      + '<div style="font-size:40px;margin-bottom:12px;">&#x1F6AB;</div>'
      + '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;margin-bottom:8px;">Verification Unavailable</div>'
...
function renderVerifModal() {
  ...
  mh += '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;margin-bottom:4px;">&#x2B50; Apply for Verification</div>';
...
window.openAddVerifWork = function() {
  ...
  mh += '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:15px;margin-bottom:4px;">Add Work #' + (_vwWorks.length + 1) + '</div>';
...
// -- Report Profile --
window.openReportProfile = function(uid, encodedName) {
  var uname = decodeURIComponent(encodedName || 'this user');
  setModal(
    '<button class="mclose" onclick="closeModal()">✕</button>'
    + '<div style="text-align:center;padding:8px 0 14px;">'
    + '<div style="font-size:40px;margin-bottom:10px;">🚩</div>'
    + '<div style="font-family:Syne,sans-serif;font-weight:800;font-size:16px;margin-bottom:4px;">Report Profile</div>'
    + '<div style="font-size:11px;color:var(--td);margin-bottom:16px;">Reporting: <strong>' + uname + '</strong></div>'
    + '</div>'
    + '<div class="fg"><label class="fl">Reason</label>'
    + '<select class="fi" id="rp-reason">'
    + '<option value="fake">Fake or impersonation</option>'
    + '<option value="harassment">Harassment or abuse</option>'
    + '<option value="spam">Spam or scam</option>'
    + '<option value="inappropriate">Inappropriate content</option>'
    + '<option value="fraud">Fraud or financial scam</option>'
    + '<option value="other">Other</option>'
    + '</select></div>'
    + '<div class="fg"><label class="fl">Details <span style="font-size:9px;color:var(--td);">(optional)</span></label>'
    + '<textarea class="fi" id="rp-detail" rows="3" placeholder="Describe what happened..." style="resize:vertical;"></textarea></div>'
    + '<button class="btn" id="rp-submit" style="width:100%;">Submit Report</button>'
  );
  setTimeout(function() {
    var btn = document.getElementById('rp-submit');
    if (!btn) return;
    btn.onclick = async function() {
      var reason = document.getElementById('rp-reason').value;
      var detail = (document.getElementById('rp-detail').value || '').trim();
      btn.disabled = true; btn.textContent = 'Submitting...';
      try {
        var rid = 'report_' + Date.now() + '_' + Math.random().toString(36).slice(2,6);
        await fbSet('profile_reports', rid, {
          id: rid, reportedUid: uid, reportedName: uname,
          reporterUid: ME.uid, reporterName: ME.name,
          reason: reason, detail: detail, status: 'pending', ts: Date.now()
        });
        closeModal();
