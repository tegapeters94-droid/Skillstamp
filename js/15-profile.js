...
function buildProfile(u,isOwn){
  ...
  var P={
    ...
    landscape: '<rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>',
  ...
  if(isClient){
    ...
    statsHtml='<div class="prf-stats-row">'
      ...
      +'<div class="prf-stat-item"><div class="prf-stat-icon-svg">'+icon('briefcase',16,'var(--td)')+'</div><div class="prf-stat-val">'+_gc+'</div><div class="prf-stat-sub">Completed Gigs</div></div>'
      +'<div class="prf-stat-div"></div>'
  ...
  var endorseSection='<div class="prf-card">'
    +'<div class="prf-card-title" style="display:flex;align-items:center;justify-content:space-between;">'
      +'<span>'+icon('heart',14,'currentColor')+' Peer Endorsements <span style="font-size:10px;font-weight:400;color:var(--td);">('+endorsements.length+')</span></span>'
      +endorseBtn
    +'</div>'
    +endorseItems
    +'</div>';
  ...
  var identityCard='<div class="prf-side-card">'
    +'<div class="prf-side-title">'+icon('lock',13,'currentColor')+' Identity</div>'
    +(isClient
  ...
  return ''
    ...
    +'<div class="prf-header-card">'
      +'<div class="prf-av-wrap">'
        ...
        +(isClient?'<div class="prf-av-role-icon">'+icon('briefcase',13,'var(--td)')+'</div>':'')
        +changePhotoBtn
      +'</div>'
      +'<div class="prf-header-info">'
        ...
        +skillIdChip
      +'</div>'
      +'<div class="prf-cta-wrap">'+ctaBtn+(isOwn?editProfileBtn:'')+(isOwn?boostProfileBtn:'')+'</div>'
    +'</div>'
