...
function buildProfile(u,isOwn){
  ...
  if(!isOwn){
    ...
    actionBtns+='<button onclick="openReportProfile(\'' + u.uid + '\',\'' + encodeURIComponent(u.name) + '\')" style="width:100%;margin-top:6px;background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.18);color:#ef4444;font-family:Plus Jakarta Sans,sans-serif;font-size:10px;font-weight:700;padding:8px 12px;border-radius:8px;cursor:pointer;">🚩 Report Profile</button>';
  }
...
window.openReportProfile = function(uid, encodedName) {
  ...
  setModal(
    ...
    + '<div style="font-family:Plus Jakarta Sans,sans-serif;font-weight:800;font-size:16px;margin-bottom:4px;">Report Profile</div>'
