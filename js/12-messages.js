...
window.openMsg = async function(uid) {
  ...
  if (mc) { mc.style.padding = '0'; mc.style.maxHeight = '90vh'; mc.style.overflow = 'hidden'; }
  ...
  try {
    ...
    if (!mc2) return;
    mc2.style.padding = '0';
    mc2.style.maxHeight = '90vh';
    mc2.style.overflow = 'hidden';
    mc2.innerHTML =
      '<div style="display:flex;flex-direction:column;height:75vh;">'
      // Header
      ...
      + '<input id="chat-input" class="fi" placeholder="Type a message…" style="flex:1;margin:0;" autocomplete="off">'
      + '<button id="chat-send" class="btn" style="padding:10px 16px;flex-shrink:0;">Send</button>'
      + '</div>'
      + '</div>';
...
window.closeModal = function() {
  if (window._chatUnsub) { try { window._chatUnsub(); } catch(e) {} window._chatUnsub = null; }
  // Restore modal default padding
  var mc = document.getElementById('mcontent');
  if (mc) { mc.style.padding = ''; mc.style.maxHeight = ''; mc.style.overflow = ''; }
