...
window.openConv=async function(cid){
  ...
  if(!pg) return;
  pg.innerHTML='<div style="display:flex;flex-direction:column;height:calc(100vh - 60px);max-height:calc(100vh - 60px);padding-bottom:65px;">'
...
window.openMsg=async function(uid){
  ...
  // Open directly as a full-screen modal chat (no page-messages needed)
  openConvModal(cid);
};


window.openConvModal=async function(cid){
  // Show loading state immediately
  setModal('<div style="display:flex;flex-direction:column;height:80vh;"><div style="padding:16px;text-align:center;color:var(--td);font-size:12px;">Loading chat…</div></div>');
  ...
  var otherId=convData.participants.find(function(p){return p!==ME.uid;});
  var other=getUser(otherId)||{name:'Unknown',gradient:'#888',title:''};

  function buildMsgsHTML(messages){
    var h='';
    messages.forEach(function(m){
      var isMe=m.from===ME.uid;
      h+='<div style="display:flex;justify-content:'+(isMe?'flex-end':'flex-start')+';margin-bottom:10px;">'
        +'<div style="max-width:75%;background:'+(isMe?'var(--acc)':'var(--s2)')+';color:'+(isMe?'#fff':'var(--fg)')+';border-radius:'+(isMe?'12px 12px 2px 12px':'12px 12px 12px 2px')+';padding:9px 13px;font-size:12px;line-height:1.5;">'
        +m.text
        +'<div style="font-size:9px;opacity:.6;margin-top:3px;text-align:'+(isMe?'right':'left')+';">'+timeAgo(m.ts)+'</div>'
        +'</div></div>';
    });
    return h || '<div style="text-align:center;color:var(--td);font-size:11px;padding:20px;">No messages yet. Say hello!</div>';
  }

  var avH=other.avatar
    ?'<img src="'+other.avatar+'" style="width:36px;height:36px;border-radius:50%;object-fit:cover;">'
    :'<div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,'+other.gradient+','+other.gradient+'88);display:flex;align-items:center;justify-content:center;font-family:Plus Jakarta Sans,sans-serif;font-weight:800;font-size:13px;color:#000;">'+initials(other.name)+'</div>';

  var chatHTML=''
    +'<div style="display:flex;flex-direction:column;height:82vh;">'
    // Header
    +'<div style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-bottom:1px solid var(--br);flex-shrink:0;">'
    +'<button onclick="closeModal()" style="background:none;border:none;color:var(--fg);font-size:20px;cursor:pointer;padding:0 6px 0 0;">←</button>'
    +avH
    +'<div style="flex:1;">'
    +'<div style="font-family:Plus Jakarta Sans,sans-serif;font-weight:700;font-size:13px;">'+other.name+'</div>'
    +'<div style="font-size:10px;color:var(--td);">'+(other.title||other.role||'Member')+'</div>'
    +'</div>'
    +'</div>'
    // Messages feed
    +'<div id="chat-modal-feed" style="flex:1;overflow-y:auto;padding:14px 14px;display:flex;flex-direction:column;">'
    +buildMsgsHTML(convData.messages)
    +'</div>'
    // Input bar
    +'<div style="display:flex;gap:8px;padding:10px 14px;border-top:1px solid var(--br);flex-shrink:0;background:var(--bg);">'
    +'<input id="chat-modal-input" class="fi" placeholder="Type a message…" style="flex:1;" data-cid="'+cid+'" data-other="'+otherId+'" onkeydown="if(event.key==='Enter') sendMsgModal(this.dataset.cid, this.dataset.other)">'
    +'<button class="hbtn" id="chat-send-btn" style="padding:10px 16px;">Send</button>'
    +'</div>'
    +'</div>';

  // Update modal with full chat UI
  var mcontent=document.getElementById('mcontent');
  if(mcontent){
    mcontent.style.padding='0';
    mcontent.style.maxHeight='90vh';
    mcontent.style.overflow='hidden';
    mcontent.innerHTML=chatHTML;
  }
  ...
  setTimeout(function(){
    ...
    if(sendBtn) sendBtn.onclick=function(){
      var el=document.getElementById('chat-modal-input');
      if(el) sendMsgModal(el.dataset.cid,el.dataset.other);
    };
  },50);
  // Scroll to bottom
  var feed=document.getElementById('chat-modal-feed');
  if(feed) feed.scrollTop=feed.scrollHeight;
  ...
  try{
    window._unsubConv=window.FB_FNS.onSnapshot(
      ...
      function(snap){
        if(!snap.exists()) return;
        var data=snap.data();
        var feed2=document.getElementById('chat-modal-feed');
        if(!feed2) return;
        var wasAtBottom=feed2.scrollHeight-feed2.scrollTop<=feed2.clientHeight+60;
        feed2.innerHTML=buildMsgsHTML(data.messages||[]);
        if(wasAtBottom) feed2.scrollTop=feed2.scrollHeight;
      }
    );
...
window.sendMsgModal=async function(cid,otherId){
  ...
  var convData=(await fbGet('conversations',cid))||{participants:[ME.uid,otherId],messages:[]};
  convData.messages.push(msg);
  convData.lastMsg=text;
  convData.lastTs=Date.now();
  await fbSet('conversations',cid,convData);
  pushNotif(otherId,'message','💬 New Message',ME.name+': '+text.slice(0,80),{type:'message',cid:cid,fromUid:ME.uid});
};
