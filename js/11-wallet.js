...
function _doRenderWallet(){
  ...
  txList.innerHTML=w.transactions.map(tx=>('<div class="tx-row">'    +'<div class="tx-icon" style="background:'+(tx.type==='in'?'rgba(74,222,128,.12)':'rgba(255,107,53,.12)')+';">'    +(tx.type==='in'        ?'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>'        :'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>')    +'</div>'    +'<div style="flex:1;"><div style="font-family:Plus Jakarta Sans,sans-serif;font-weight:600;font-size:12px;">'+tx.desc+'</div><div style="font-size:10px;color:var(--td);">'+(tx.from||'SkillStamp')+' · '+timeAgo(tx.ts)+'</div></div>'    +'<div class="tx-amount '+(tx.type==='in'?'tx-in':'tx-out')+'">'+(tx.type==='in'?'+':'-')+'$'+tx.amount.toLocaleString()+'</div>'    +'</div>')).join('');
...
window.openReceive=function(){
  setModal('<button class="mclose" onclick="closeModal()">✕</button>'    +'<h3>Receive Payment</h3>'    +'<p>Share your SkillID or QR code with clients to receive payments directly.</p>'    +'<div style="background:var(--s2);border:1px solid var(--br);border-radius:var(--r);padding:16px;text-align:center;margin-bottom:16px;">'    +'<div style="font-size:11px;color:var(--td);margin-bottom:6px;">Your Wallet Address</div>'    +'<div style="font-family:Inter,sans-serif;font-size:16px;color:var(--gld);letter-spacing:.1em;">'+ME.skillId+'</div>'    +'</div>'    +'<div id="receive-qr" style="display:flex;justify-content:center;margin-bottom:16px;"><div style="background:white;padding:12px;border-radius:8px;" id="rqr"></div></div>'    +'<button class="btn" onclick="navigator.clipboard&&navigator.clipboard.writeText(\''+ME.skillId+'\').then(()=>toast(\'Wallet address copied!\'))">Copy Wallet Address</button>');
