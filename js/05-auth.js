...
window.addEventListener('load',()=>{
  ...
  if(sid){
    // Show a minimal loading state while we fetch data
    var lScreen=document.getElementById('screen-loading');
    if(lScreen) lScreen.classList.add('active');
    // Load user + all required cache before entering app
    fbGet('users', sid).then(async function(u){
      if(u){
        ME=u;
        if(ME.isBanned||ME.badgeStatus==='suspended'){
          LOCAL.del('session');
          if(lScreen) lScreen.classList.remove('active');
          return;
        }
        try {
          ...
          await Promise.all([
            loadGigsToCache(),
            loadEndorsementsToCache ? loadEndorsementsToCache() : Promise.resolve(),
            // Load users into cache (re-use existing fbGetAll)
          ...
          console.warn('Cache load on resume failed, continuing anyway', e);
        }
        if(lScreen) lScreen.classList.remove('active');
        enterApp();
        showPage('home');
