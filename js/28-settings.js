...
// Keeps settings panel alive — opens actions as modals ON TOP
// User can close modal and return to settings without losing state
window.settingsNav = function(action) {
  // Map action → function to call
  // Settings panel stays in DOM; modals layer on top
  var actions = {
  ...
  if (fn) {
    // Small delay so tap feedback registers before modal opens
    setTimeout(fn, 80);
  }
};
