...
window.openHireModal=async function(gid){
  ...
  applicants.forEach(function(uid){
    ...
    rows+=avHTML(u,36,'50%');
    rows+='<div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:700;font-family:Syne,sans-serif;">'+u.name+'</div>';
    ...
    // Proposal cover note
    if(cover){
      rows+='<div style="padding:10px 12px;border-top:1px solid var(--br);background:var(--bg);">';
      rows+='<div style="font-size:9px;font-weight:700;color:var(--tx);text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px;">📋 Cover Note</div>';
      rows+='<div style="font-size:11px;color:var(--td);line-height:1.75;">'+cover+'</div>';
      rows+='</div>';
      // Show screening answer if present
      var sAns = proposal && proposal.screeningAns ? proposal.screeningAns : '';
      if (sAns) {
        rows+='<div style="padding:10px 12px;border-top:1px solid var(--br);background:rgba(232,197,71,.04);">';
        rows+='<div style="font-size:9px;font-weight:700;color:var(--gld);text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px;">📋 Screening Answer</div>';
        rows+='<div style="font-size:11px;color:var(--tx);line-height:1.65;">'+sAns+'</div>';
        rows+='</div>';
      }
    }
    rows+='</div>';
...
window.confirmComplete=async function(gid){
  ...
  try{
    ...
    setModal('<div style="text-align:center;padding:10px 0 6px;">'
      ...
      +'<div style="font-family:Syne,sans-serif;font-weight:800;font-size:18px;margin-bottom:6px;">Payment Released!</div>'
